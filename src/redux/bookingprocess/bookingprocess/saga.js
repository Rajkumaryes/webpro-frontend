...
export const bookingService = {
  ...
  return fetch(  `${apiUrlBooking}${BookingProcess}booking`, requestOptions)
  .then(response => {
    if(!response.ok) 
    {
...
export function createbooking( id,gsc_userid,exception_raised_date,start_time,end_time,region,area,team,booking_number,csb_office,assigned_user,cu_matchcode ,mr_matchcode ,
...
return fetch(  `${apiUrlBooking}${BookingProcess}booking/create`, requestOptions)
.then(response => {
  if(!response.ok) 
  {
    if(response.status === 401)
...
return fetch(  `${apiUrlBooking}${BookingProcess}booking/search`, requestOptions)
.then(response => {
  if(!response.ok) 
  {
    if(response.status === 401)
...
export function fetchbookingcount(gsc_userid) {
  const requestOptions = {
    method: 'POST',
    headers: {
        'Accept': 'application/json, text/plain, */*',  // It can be used to overcome cors errors
...
return fetch(  `${apiUrlBooking}${BookingProcess}booking/bookingcount`, requestOptions)
...
export function fileUpload(file)
{
  ...
  var url = `${apiUrl}${BookingProcess}booking/bulkupload`
