...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    MisactivitytrackService.deletemisactivity(id)
    .then((res) => {
      if(res){
...
      })
  ...
  onChangeFileUpload(files)
    ...
    MisactivitytrackService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Track & Trace Activity',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Track & Trace Activity',languageData)} match={match} />
                    </div>
