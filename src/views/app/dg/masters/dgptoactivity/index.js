...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    MisactivityService.deletemisactivity(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="DG-PTO Activity.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    MisactivityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'DG-PTO Activity',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'DG-PTO Activity',languageData)} match={match} />
                    </div>
