...
onSubmit() { 
  const {id,region_id,name,status,isEdit} = this.state;
...
createAPI(name,status)
{
  ...
  AreaService.createarea(name,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,status)
{
  ...
  AreaService.updatearea(id,name,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {region_id,name,region_data,isEdit,status,loading,area_data} = this.state
