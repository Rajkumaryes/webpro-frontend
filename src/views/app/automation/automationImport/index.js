...
      })
      return(
        ...
        <Workbook filename="requesttype.xlsx" element={
          ...
          <Workbook.Column label="Request" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    uploadService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Import',languageData),
                dataIndex: 'name',
                key: 'name',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Import',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Import',languageData)} match={match} />
                    </div>
