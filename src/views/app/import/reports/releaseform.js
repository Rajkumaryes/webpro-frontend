...
class QueryResolveSheet extends Component {
    ...
    fetchPagination(page,per_page,startdate,enddate,is_report)
    {
        ...
        if(enddate !== "" && startdate !== ""  && team_id !== "")
        {
          ...
          if ((Date.parse(enddate) > Date.parse(startdate)))
          {
            ...
            const report_url =  'releasecount',
            menu= 'Imports' , 
            submenu='Release Count Sheet',
            type = 'Raw Data'
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,charge_data,team_id,team_data,is_submit,loading,data,page,pageSize,
          type_data,area_data} = this.state
        var columns= [
          {
          ...
          {
            title: onChangeLanguage(locale,'Charges',languageData),
            dataIndex: 'charges',
            key: 'charges',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {getValue(charge_data,'value','label',text)}    
              </div>),           
          },
          {
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Release Count Sheet Raw Data',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <Breadcrumb heading={onChangeLanguage(locale,'Release Count Sheet Raw Data',languageData)} match={match} />
