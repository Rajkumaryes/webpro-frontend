...
class MasterPage extends Component {
        ...
        if(res.status)
          {
            let filterstatus = (res.data).filter(item => item.status === 1)
             var arealist = filterstatus.map(function(response) {
                  return  {text : response.name ,value : response.id.toString()};
               });  
            this.setState({ 
  ...
  renderTemplate()
  {
    ...
    const {data,error_code_data} = this.state
    var array = data.map(record=> {
          return {
            'name' : record.name,
            'error_code' : getValue(error_code_data,'value','text',record.error_code)
        };
      })
      return(
        
        <Workbook filename="Subactivity.xlsx" element={
          ...
          <Workbook.Column label="Errorcode" value="error_code"/>
          <Workbook.Column label="SubActivity" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
        ...
        const columns = [
            {
              ...
              render: (text, record) => ( 
                <span className="row d-flex justify-content-center">                             
                 { getValue(error_code_data,'value','text',text)}
                </span>
                 ),  
                 filters:error_code_data,
                filterMultiple: false,
                onFilter: (value, record) => record.error_code === value,
            },
             {
