       ...
       if(res.status)   { 
          let filterstatus = (res.data).filter(item => item.status === 1)
             var regionlist = filterstatus.map(function(cusmaidid) {
                  return  {text : cusmaidid.area_name ,value : cusmaidid.id.toString()};
               });  
                this.setState({
...
          return {
            'type' : record.type,
            'team' : record.team !== null ? getValue(area_data,'value','text',record.team) :  '',

        };
      })
      return(
        
        <Workbook filename="Checklist.xlsx" element={
          ...
          <Workbook.Column label="Name" value="type"/>
  ...
  onChangeFileUpload(files)
    ...
    checklistService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
          {
            ...
            render: (text, record) => ( 
              <span className="row d-flex justify-content-center">                             
                    {getValue(area_data,'value','text',text)}
              </span>
               ),  
               filters: area_data,
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Check List',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Check List',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Check List',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Check List' : 'Create Check List'}   
          </ModalHeader>
          <ModalBody>
