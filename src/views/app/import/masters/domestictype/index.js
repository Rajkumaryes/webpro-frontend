...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      domestictypeService.fetchdomestictype()
        .then((res) => { 
          this.setState({   
        loading : false 
  ...
  handleDelete = id => {
    domestictypeService.deletedomestictype(id)
    .then((res) => {
      if(res){
...
          return {
            'name' : record.name,
            'unit' : record.unit,
            'area_id' :record.area_id !== null? getValue(area_Data,'value','text',record.area_id) : '' ,

        };
      })
      return(
        
        <Workbook filename="Type.xlsx" element={
          ...
          <Workbook.Column label="Type" value="name"/>
          <Workbook.Column label="EQ" value="unit"/>
  ...
  onChangeFileUpload(files)
    ...
    domestictypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Domestic Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Type',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Type',languageData)}
                      </Button>
                    </div>
