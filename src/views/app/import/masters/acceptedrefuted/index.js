...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    acceptedrefutedService.deleteacceptedrefuted(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="AcceptedRefuted.xlsx" element={
          ...
          <Workbook.Column label="Accepted Refuted" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    acceptedrefutedService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Accepted Refuted',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Accepted Refuted',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Accepted Refuted',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Accepted Refuted' : 'Create Accepted Refuted'}   
          </ModalHeader>
          <ModalBody>
