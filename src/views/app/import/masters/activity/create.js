...
onSubmit() { 
  const {id,name,unit,status,team,isEdit} = this.state;

  if(name !== "" && unit !== '' &&team !== '')
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,unit,team,status)
    }
    else
    {
      this.createAPI(name,unit,team,status)
    }
...
createAPI(name,unit,team,status)
{
  this.setState({
    loading : true
  })
  activityService.createactivity(name,unit,team,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,unit,team,status)
{
  this.setState({
    loading : true
  })
  activityService.updateactivity(id,name,unit,team,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {name,isEdit,status,unit,loading,team_data,team} = this.state
