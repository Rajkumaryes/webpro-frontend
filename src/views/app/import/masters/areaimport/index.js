...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    areaimportService.deleteareaimport(id)
    .then((res) => {
      if(res){
...
          return {
            'Region' :record.region_id !== null ? getValue(region_data,'value','text',record.region_id):'' ,
            'Team' : record.area_name,
        };
      })
      return(
  ...
  onChangeFileUpload(files)
    ...
    areaimportService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
