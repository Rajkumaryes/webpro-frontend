...
class MasterPage extends Component {
         ...
         if(res.status)   { 
             let filterstatus = (res.data).filter(item => item.status === 1)
               var arealist = filterstatus.map(function(response) {
                    return  {text : response.area_name ,value : response.id.toString()};
                 });  
                  this.setState({
                  area_Data :  arealist,
                  loading:false
  ...
  handleDelete = id => {
    deliveryorder_teamService.deletedeliveryorder_team(id)
    .then((res) => {
      if(res){
  ...
  renderTemplate()
  {
    ...
    const {data,area_Data} = this.state
    var array = data.map(record=> {
          return {
            'unit' : record.unit,
            'team_id' :record.team_id !== null? getValue(area_Data,'value','text',record.team_id) : '' ,

        };
      })
      return(
        
        <Workbook filename="Delivery Order Query Unit.xlsx" element={
          ...
          <Workbook.Column label="Team" value="team_id"/>
  ...
  onChangeFileUpload(files)
    ...
    deliveryorder_teamService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
          {
            ...
            render: (text, record) => (	
              <div>
                 {text !== null &&getValue(area_Data,'value','text',text)}
              </div>
          ),
          filters: area_Data,
          filterMultiple: false,
          onFilter: (value, record) => record.team_id === value,
          },
            
              {
                title:  onChangeLanguage(locale,'EQ/Unit',languageData),
                dataIndex: 'unit',
                key: 'unit',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Delivery Order Query Unit',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Delivery Order Query Unit',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Unit',languageData)}
                      </Button>
                    </div>
