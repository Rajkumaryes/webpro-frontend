...
onSubmit() {
  const {id,unit,team_id,status,isEdit} = this.state;

  if(unit !== ""&& team_id !== "")
  {
    if(isEdit === true)
    {
      this.updateAPI(id,unit,team_id,status)
    }
    else
    {
      this.createAPI(unit,team_id,status)
    }
...
createAPI(unit,team_id,status)
{
  this.setState({
    loading : true
  })
  deliveryorder_teamService.createdeliveryorder_team(unit,team_id,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,unit,team_id,status)
{
  this.setState({
    loading : true
  })
  deliveryorder_teamService.updatedeliveryorder_team(id,unit,team_id,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {unit,team_id,isEdit,status,area_Data,loading} = this.state
           ...
           <Row>
              ...
              <Colxx xxs="8" className="mb-4">
                  <Select className = "fontstyle"
                    ...
                    value={area_Data.filter(option =>option.value === team_id)}
                    options={area_Data}
                    onChange={({value}) => this.setState({  team_id: value })}
                  />
               </Colxx>
             ...
             <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label> {onChangeLanguage(locale,'EQ/Unit',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
               
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'EQ/Unit',languageData)} 
                        value = {unit}  
