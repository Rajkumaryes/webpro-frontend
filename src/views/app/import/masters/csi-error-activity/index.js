...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    csierroractivityService.deletecsierroractivity(id)
    .then((res) => {
      if(res){
...
      })
  ...
  onChangeFileUpload(files)
    ...
    csierroractivityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'csierroractivity',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
