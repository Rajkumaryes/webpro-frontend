...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    customstypeService.deletecustomstype(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Customstype.xlsx" element={
          ...
          <Workbook.Column label="Custom type" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    customstypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
