...
import{budgetedService} from '../../../../redux/kfr_report/budgeted/saga'
...
class QueryResolveSheet extends Component {
    ...
    fetchPagination(page,per_page,region,area,team,process,month,year,location,isdoc,is_report)
    {
      ...
      const date_time = moment(new Date()).format('MM/DD/YYYY hh:mm:ss a')
        ...
        year !=='' || location !==''  )
        {
            ...
            budgetedService.filter(page,per_page,region,area,team,process,month,year,location,isdoc,is_report,username,date_time)
            .then((res) => { 
              this.setState({   
                   loading : false   
              }) 
  ...
  onClickTab(menu)
 {
  ...
        })
        return(
          ...
          <Workbook filename="Budgeted HC.xlsx" element={
 ...
 { 
    ...
    if(process !== '' && region !== '' && area !== '' && team !== '' && year !== '' && month !== '')
    {
      ...
      budgetedService.updateData(id,page,per_page,region,area,team,process,month,year,location,isdoc,value,username)
      .then((res) => { 
        this.setState({   
          loading : false     
        }) 
  ...
  const {region,area,team,process,month,year,location,isdoc} =this.state
    ...
    budgetedService.fileUpload(files[0],region,area,team,process,month,year,location,isdoc)
      .then((res) => { 
        this.setState({
          loading : false
        })
    ...
    render()
    {
        ...
        var columns= [
          ...
          },
          {
            ...
            key: 'isdoc',
            render: (text, record,index) => (
              <span className="row d-flex justify-content-center" style = {{}}>                             
                  <Row>
                    <Colxx xxs="12">
                        <CustomRadioButton 
                        checked  = {true} 
                        name = {onChangeLanguage(locale,'Doc',languageData)} 
                        value = {text} 
                        disabled={!is_edit}
                    ...
                    <Colxx xxs="12">
                      <CustomRadioButton 
                      checked  = {false} 
                      name = {onChangeLanguage(locale,'Non Doc',languageData)} 
                      value = {text } 
                      disabled={!is_edit}
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Budgeted HC',languageData)}</title>
            <Row>
              <Colxx xxs="12">
              <div className = "row">
             <div className = "col-md-8">
             <Breadcrumb heading={onChangeLanguage(locale,'Budgeted HC',languageData)} match={match} />
             </div>
             {is_edit === true &&
