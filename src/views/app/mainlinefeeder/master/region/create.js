...
onSubmit() { 
  const {id,name,start_time,end_time,start_time_am,end_time_am,status,isEdit,shift_timings} = this.state;

  if(name !== "")
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,shift_timings,status)
    }
    else
    {
      this.createAPI(name,shift_timings,status)
    }
...
createAPI(name,shift_timings,status)
{
  const {username} = this.props
  this.setState({
    loading : true
  })
  regionService.createregion(name,username,shift_timings,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,shift_timings,status)
{
  this.setState({
    loading : true
  })
  regionService.updateregion(id,name,shift_timings,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {name,shift_timings,region_data,start_time,end_time,starthour,endhour,start_time_am,end_time_am,starthourdata,endhourdata,isEdit,status,loading} = this.state
      ...
      <div>
          ...
          <div>
            ...
            {/* <Select  
                 ...
                 <Colxx xxs="4" className="mb-4">
                   <Label> {onChangeLanguage(locale,'Shift Timings',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
               
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Shift Timings',languageData)} 
                        value = {shift_timings}  
                        onChange= {(e)=>this.setState({shift_timings : e.target.value})} ></Input>
                 </Colxx>
             </Row>
             {isEdit && 
                <Row>
