...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    ssydetailsService.deletessydetails(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="SSYDetails.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    ssydetailsService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'SSY Details',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'SSY Details',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add SSY',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update SSY' , languageData) : onChangeLanguage(locale,'Create SSY' , languageData) }   
           </ModalHeader>
          <ModalBody>
