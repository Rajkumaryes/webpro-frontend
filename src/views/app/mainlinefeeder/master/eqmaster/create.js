...
import{eqmasterService} from '../../../../../redux/mainlinefeeder/master/eqmaster/saga'
...
onSubmit() { 
  const {id,name,region,eq,status,isEdit} = this.state;
  ...
  if(eq !== "" && name !== '' && region !== '')
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,region,eq,status)
    }
    else
    {
      this.createAPI(name,region,eq,status)
    }
...
createAPI(name,region,eq,status)
{
  ...
  eqmasterService.createeqmaster(name,region,eq,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,region,eq,status)
{
  this.setState({
    loading : true
  })
  eqmasterService.updateeqmaster(id,name,region,eq,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
onChangeRegion(value)
{
  ...
  render()
  {
    ...
    const {region,name,eq,region_data,isEdit,status,loading} = this.state
