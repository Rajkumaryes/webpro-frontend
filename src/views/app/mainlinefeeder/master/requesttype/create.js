...
onSubmit() { 
  const {id,name,tat_limit,status,isEdit} = this.state;

  if(name !== "")
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,tat_limit,status)
    }
    else
    {
      this.createAPI(name,tat_limit,status)
    }
...
createAPI(name,tat_limit,status)
{
  this.setState({
    loading : true
  })
  requestService.createrequest(name,tat_limit,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,tat_limit,status)
{
  this.setState({
    loading : true
  })
  requestService.updaterequest(id,name,tat_limit,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
handleHourChange = (e) => {
  this.setState({ tat_limit: e.target.value });
};
  render()
  {
    ...
    const {name,isEdit,status,loading,tat_limit} = this.state
    ...
    return (
      <div>
            ...
            <Loading loading={true} background="rgba(239,100,50, 0.1)" loaderColor="#EF6432" />
             ...
             <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label> {onChangeLanguage(locale,'Request Type',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
               
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Request Type',languageData)} 
                        value = {name}  
          ...
          <Colxx xxs="4" className="mb-4">
            <Label>
              {onChangeLanguage(locale, 'TAT Limit', languageData)}
              <a style={{ color: 'red' }}>*</a>
            </Label>
          </Colxx>
          ...
          <Colxx xxs="8" className="mb-4">
            ...
            >
              <option value="" disabled>{onChangeLanguage(locale, 'Select Hours', languageData)}</option>
              {hourOptions}
            </Input>
          </Colxx>
        </Row>
