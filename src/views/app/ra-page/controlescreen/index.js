...
import {onChangeLanguage,getValue,getTimeDifference,getAreaValue,getEnableStatus,
  getStausOptionAdminEnable,getPermission,
...
class QueryResolveSheet extends Component {
      ...
      auditor_status,size,priority,area,region,contract_no,is_create)
    {
      ...
      amdcontractsService.filtercontrolapi(page,per_page,start_date,end_date,request_no,publisher,publisher_status,auditor,
    ...
    onSubmitFilter(is_create)
    {
      const {page,pageSize, start_date,end_date,request_no,publisher,publisher_status,auditor,
             auditor_status,size,priority,area,region,contract_no} = this.state
    ...
    onClickDetails(id){
          if(id !== -1)
          {
            ...
            this.props.history.push(`${adminRoot}/ra/details-box/controle/${id}`)
          }
          else
          {
   ...
   getCount(key,data)
   {
    ...
    render()
    {
        ...
        publisher,publisher_status,auditor,select_index,auditor_status,size,priority,area,region,is_submit_table} = this.state
            ...
            },
            {
                ...
                render: (text, record,index) => ( 
                  <div style = {{padding:'2px',width:'100px'}}>
                     <Input   className = {(is_submit_table === true && text === "" && typeof record.id === 'string' )   ?  "error-border" :"fontstyle"  } 
                        value = {text} 
                ...
                render: (text, record,index) => ( 
                  <div  style = {{padding:'2px',width:'100px'}}>
                    <Select   className="react-select fontstyle"
                        ...
                        options={user_data}
                        isDisabled = {getEnableStatus('publisher',user_data,record.request_no,record.publisher,record.publisher_status,record.auditor)}
                ...
                render: (text, record,index) => ( 
                  <div  style = {{padding:'2px',width:'100px'}}>
                    <Select   className="react-select fontstyle"
                        ...
                        options={this.getStatustime(text) }
                        styles={customStyles}
        ...
        //    columns.push({
        //     title: onChangeLanguage(locale,'TAT Initimation',languageData),
        //     key: 'id',
        //    dataIndex: 'id',
        //     render: (id,record) => (
        //      <div style = {{padding:'2px'}}>
        //         <Button color = "secondary" className="fontstyle detail-button-width" 
        //           onClick={()=>this.onClickDetails(record.id)}
        //               >
        //           {onChangeLanguage(locale,'Details',languageData)} 
        //         </Button>
        //      </div>
        //     )
        //  })
           ...
           dataIndex: 'statics',
            render: (text,record) => (
                <div  style = {{padding:'2px',width:'100px'}} >
                    ...
                    value = {text}  
                    disabled = {true}
                    onChange={(e) =>this.onChangeValue_row(e.target.value,record.id,'statics')} 
                    /> */}
                    {text}
               </div>
            )
        ...
        const pagination = {
          ...
          pageSize: this.state.pageSize,
            ...
            return (
            <>
            <title>{onChangeLanguage(locale,'Control Screen',languageData)}</title>
            {loading && 
              <div>
            ...
            <Row>
              ...
              <div className = "row">
                      ...
                      <Breadcrumb heading={onChangeLanguage(locale,'Control Screen',languageData)} match={match} />
                    </div>
    ...
    setPendingtime()
    {
      ...
      if(modal_index !== -1 && is_publish !== '' && pending_out !== '')
      {
        ...
        {
           startdate = aud_pending_in !== '' ? new Date(aud_pending_in) : new Date()
                            ...
                            <Row>
                                    ...
                                    <Colxx xxs="10">
                                            ...
                                            {size_data && size_data.map((value,index) =>
                                             <Colxx xxs="2" className = "text-center">  <Label  className = "fontstyle to-small-font" >{value.label}</Label></Colxx>
                                            )}
                                             <Colxx xxs="2" className = "text-center">  <Label  className = "fontstyle to-small-font" >{onChangeLanguage(locale,'Total',languageData)}</Label></Colxx> 
                                            </Row>
                                      </Colxx>
