...
class QueryResolveSheet extends Component {
    ...
    getTimebasedonstatus(id,key)
    {
      ...
      onClickerrorlog()
      {
        ...
        this.props.history.push(`/app/ra/correction-log`)
      }
      onClickCurrenrecord()
      {
      
        localStorage.setItem('amd_contract_id' , this.state.request_no)
        this.props.history.goBack()
      }
  ...
  onSubmit()
  {
    const{id,request_no,contract_no,type_of_req,amd_no,priority,size,area,region
      ...
      if(isfill === true)
      {
          var aud_total = '',pub_total = '',updated_end_time= convertLocalToUTCDate(new Date()),type_of_reqs = type_of_req
          ...
          this.setState({
            ...
            if(amd_no !== '' && amd_no !== null)
            {
              
                if(parseInt(amd_no) === 0)
                {
                  type_of_reqs  = 'Original'
                }
                else if((amd_no).toLowerCase() === 'dummy')
                {
                  type_of_reqs  = 'Internal Adjustment'
                }
                else if(amd_no.toLowerCase() === 'error')
                {
                  type_of_reqs  = 'Correction'
                }
                else 
                {
                  type_of_reqs  = 'Amendment'
                }
            }
            else
            {
        ...
        amdcontractsService.updateapi(id,request_no,contract_no,type_of_reqs,amd_no,priority,size,area,region
            ,received_time,publisher,publisher_status,pub_in_progress,pub_pending_in,pub_pending_out,
  ...
  handleDelete(id)
  {
    ...
    amdcontractsService.deleteapi(id)
    .then((res) => {
      if(res){
        this.setState({   
            loading : false     
  ...
  getDisabled(key,data)
  {
    ...
    const is_adimin_Edit = getPermission(role_permission_data,'RA','Details Box',true)
    var isDisabled = true
  ...
  onChange_routes1(e){
    ...
    if (e.target.value === '' || re.test(e.target.value)) {
       this.setState({no_of_rutes: e.target.value})
    }
 ...
 handleKeypress (e) {
  ...
  if (characterNumber >= 0 && characterNumber <= 9) {
    ...
    render()
    { 
        const{loading,id,user_data,size_data,priority_data,status_data,request_no,contract_no,amd_no,priority,size
        ,received_time,publisher,publisher_status,pub_in_progress,pub_pending_in,pub_pending_out,
        pub_done_time,pub_disregrads,pub_total,pub_routes,comments_publisher,auditor,
        auditor_status,aud_in_progress,aud_pending_in,aud_pending_out,error_log,
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Details Box',languageData)}</title>
            {loading && 
              <div>
            ...
            <Row>
              ...
              <div className = "row">
                      ...
                      <Breadcrumb heading={onChangeLanguage(locale,'Details Box',languageData)} match={match} />
                    </div>
                  </div>
            ...
            <div className = "row" style = {{}}>
                    ...
                    <div className = "col-md-2 space-margin"  >
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Contract No',languageData)}<a style = {{color :'red'}}>*</a></Label>
                            <Input  className = {is_submit === true && contract_no === ''?  "error-border":"fontstyle text-background" } 
                            placeholder = ''
                            value = {contract_no}  
                        ...
                        <div className = "col-md-2 space-margin"  >
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'AMD No',languageData)}<a style = {{color :'red'}}>*</a></Label>
                                <Input  className = {is_submit === true && amd_no === ''?  "error-border":"fontstyle text-background" }
                                  placeholder = ''
                                  value = {amd_no}  
                                  onChange={(e) =>this.setState({amd_no:e.target.value})}
                                  disabled = {!is_adimin_Edit}
                                  />
                        </div>
                        <div className = "col-md-2 space-margin"  >
