...
import{crossborderService} from '../../../../redux/ra/crossborder/saga'
...
class QueryResolveSheet extends Component {
    ...
    onSubmitData(value,is_new)
    {
      ...
      const {status_data} = this.state
       ...
       var record = Object.assign({}, value);
        ...
        if(record.booking_no   === null || record.booking_no  === "" || record.received_time  === null || record.received_time === "")
        {
          isfill = false
        }
         ...
         if(isfill === true)
        {
          ...
          if(record.publisher_status === null || record.publisher_status === '')
          {
           ...
           crossborderService.multiupdate_api([record])
            .then((res) => { 
              this.setState({   
                loading : false     
              }) 
    ...
    fetchPagination(page,per_page,publisher,cc_no,request_no,booking_no,publisher_status)
    {
      this.setState({
        loading : true,
       
      })
      crossborderService.filterapi(page,per_page,publisher,cc_no,request_no,booking_no,publisher_status)
        .then((res) => { 
          this.setState({   
            loading : false,
            pageSize : per_page,
    ...
    onSubmitFilter()
    {
      const {page,pageSize, publisher,cc_no,request_no,booking_no,publisher_status} = this.state
    var isfill = false
    if(publisher !== '' || cc_no !== '' || request_no !== "" || booking_no !== '' ||publisher_status !== '')
    {
        isfill = true  
    }
      ...
      if(isfill === true)
      {
        ...
        this.fetchPagination(page,pageSize, publisher,cc_no,request_no,booking_no,publisher_status)
      }
      else
      {
    ...
    handleDelete(id)
    {
      ...
      crossborderService.deleteapi(id)
      .then((res) => {
        if(res){
          this.setState({   
              loading : false,
    ...
    { 
      ...
      const index = data.findIndex(x => x.id === id)
        if(index >= 0)
        {
            ...
            if(key === 'shipper' || key === 'publisher' || key === 'publisher_status')
            {
              var is_new = false
              if(typeof id == 'string')
              {
    ...
    paginationOptions = {
      ...
      onChange: (page,pageSize) => {
        const {publisher,cc_no,request_no,booking_no,publisher_status,is_search} = this.state
        if(is_search === true)
        {
          this.fetchPagination(page,pageSize, publisher,cc_no,request_no,booking_no,publisher_status)
        }
    ...
    render()
    {
        ...
        const {data,loading,selectedRowKeys,status_data,user_data,publisher,
          cc_no,select_index,request_no,booking_no,publisher_status ,is_submit_table} = this.state
            const columns = [
              {
              ...
              },
              {
                ...
                render: (text, record) => (	
                 ...
                 {
                   title:  onChangeLanguage(locale,'Booking No',languageData),
                   ...
                   render: (text, record) => (	
                    <div style = {{padding:'2px',width:'100px'}}>
                      <Input  className = {(is_submit_table === true && text === "" )   ?  "fontstyle error-border" :"fontstyle"  }
                          value = {text}  
                          onKeyDown ={(e)=>this.onKeyDownValue_row(e,record,'booking_no')} 
                           onKeyPress = {(e)=>this.onKeyPressValue_row(e,record,'booking_no')}
                          onChange={(e) =>this.onChangeValue_row(e.target.value,record.id,'booking_no')} 
                          />
                    </div>
                    ),
                 },
                 {
                  title: onChangeLanguage(locale,'CC/RA NO',languageData),
                  dataIndex: 'cc_no',
                  key: 'cc_no',
                  render: (text, record,index) => ( 
                    <div style = {{padding:'2px',width:'100px'}}>
                       <Input  className = "fontstyle"
                            value = {text}  
                            onKeyDown ={(e)=>this.onKeyDownValue_row(e,record,'cc_no')} 
                            onKeyPress = {(e)=>this.onKeyPressValue_row(e,record,'cc_no')}
                            onChange={(e) =>this.onChangeValue_row(e.target.value,record.id,'cc_no')} 
                            />
          
                    </div>),
                }, 
                {
                  title: onChangeLanguage(locale,'Tariff',languageData),
                  dataIndex: 'tarrif',
                  key: 'tarrif',
                  render: (text, record,index) => ( 
                    <div style = {{padding:'2px',width:'100px'}}>
                        <Input  className = "fontstyle"
                          value = {text}  
                          onKeyDown ={(e)=>this.onKeyDownValue_row(e,record,'tarrif')} 
                          onKeyPress = {(e)=>this.onKeyPressValue_row(e,record,'tarrif')}
                          onChange={(e) =>this.onChangeValue_row(e.target.value,record.id,'tarrif')} 
                          />
                    </div>),
                },
              // {
              //   title:onChangeLanguage(locale,'Shipper',languageData) ,
              //   dataIndex: 'shipper',
              //   key: 'shipper',
              //   render: (text, record,index) => ( 
              //     <div  style = {{padding:'2px',width:'100px'}}>
              ...
              //           options={user_data}
              //           onChange={({value}) =>this.onChangeValue_row(value,record.id,'shipper')} /> 
                ...
                render: (text, record,index) => ( 
                  <div  style = {{padding:'2px',width:'100px'}}>
                    <Select   className="react-select fontstyle"
                        ...
                        value={user_data.filter(option =>option.value === text)}
                        options={user_data}
                        isDisabled = {getEnableStatus('publisher',user_data,record.request_no,record.publisher,record.publisher_status,'')}
                ...
                render: (text, record,index) => ( 
                  <div  style = {{padding:'2px',width:'100px'}}>
                    <Select   className="react-select fontstyle"
                        ...
                        styles={customStyles}
                        isDisabled = {getEnableStatus('publisher_status',user_data,record.request_no,record.publisher,record.publisher_status,'')}
              ...
              {
                title:onChangeLanguage(locale,'Reason for Query',languageData) ,
                dataIndex: 'reason_query',
                key: 'reason_query',
                render: (text, record,index) => ( 
                  <div  style = {{padding:'2px',width:'100px'}}>
                      <Input  className = "fontstyle" 
                            value = {text}  
                            onKeyDown ={(e)=>this.onKeyDownValue_row(e,record,'reason_query')} 
                            onKeyPress = {(e)=>this.onKeyPressValue_row(e,record,'reason_query')}
                            onChange={(e) =>this.onChangeValue_row(e.target.value,record.id,'reason_query')} 
                            /> 
        ...
        const pagination = {
          ...
          pageSize: this.state.pageSize,
            ...
            return (
            <>
            <title>{onChangeLanguage(locale,'Cross Borders',languageData)}</title>
            {loading && 
              <div>
            ...
            <Row>
              ...
              <div className = "row">
                      ...
                      <Breadcrumb heading={onChangeLanguage(locale,'Cross Borders',languageData)} match={match} />
                    </div>
                    </div>
                ...
                <div className = "col-md-12">  
                    <div className = "row" style = {{padding:'10px'}}>
                       <div className = "col-lg-2-0 space-margin"  >
                        ...
                        <div className = "col-lg-2-0 space-margin"  >
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'CC/RA No',languageData)}</Label>
                            <Input  className = "fontstyle text-background"  
                            value = {cc_no}  
                            onChange={(e) => this.setState({  cc_no: e.target.value })}
          ...
          >
            ...
            <ModalBody>
               <div className = "col-md-12 space-margin"  >
                    ...
                    min="1"
                    value = {no_of_rutes}  
                    onChange= {(e)=>this.onChange_routes(e)} 
                    />
                </div>
             <Colxx xxs="12"> 
