...
import{amendmentsService} from '../../../../../redux/ra/amendments/saga'
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    amendmentsService.deleteapi(id)
    .then((res) => {
      if(res){
  ...
  onChangeFileUpload(files)
    ...
    amendmentsService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="amendments.xlsx" element={<Button  className = "button-width" color="secondary">
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Type of Amendments',languageData)}</title>
            <Row>
              <Colxx xxs="12">
              <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Type of Amendments',languageData)} match={match} />
                    </div>
                     <div className = "col-md-5">
                      ...
                      <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Type of Amendments',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Type of Amendments',languageData) : onChangeLanguage(locale,'Create Type of Amendments',languageData)}   
          </ModalHeader>
          <ModalBody>
