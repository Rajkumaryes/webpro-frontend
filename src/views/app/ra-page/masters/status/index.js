...
class MasterPage extends Component {
  ...
  onChangeFileUpload(files)
    ...
    statusService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {  this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="status.xlsx" element={<Button  className = "button-width" color="secondary">
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Status',languageData)}</title>
            <Row>
              <Colxx xxs="12">
              <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Status',languageData)} match={match} />
                    </div>
                     <div className = "col-md-4">
                      ...
                      <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Status',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Status',languageData) : onChangeLanguage(locale,'Create Status',languageData)}   
          </ModalHeader>
          <ModalBody>
