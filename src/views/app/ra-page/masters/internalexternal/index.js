...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    internalexternalService.deleteapi(id)
    .then((res) => {
      if(res){
  ...
  onChangeFileUpload(files)
    ...
    internalexternalService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="internalexternal.xlsx" element={<Button  className = "button-width" color="secondary">
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Internal / External',languageData)}</title>
            <Row>
              <Colxx xxs="12">
              <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Internal / External',languageData)} match={match} />
                    </div>
                     <div className = "col-md-5">
                      ...
                      <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Internal / External',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Internal / External',languageData) : onChangeLanguage(locale,'Create Internal / External',languageData)}   
          </ModalHeader>
          <ModalBody>
