...
class MasterPage extends Component {
  ...
  onChangeFileUpload(files)
    ...
    errortypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="errortype.xlsx" element={<Button  className = "button-width" color="secondary">
      {onChangeLanguage(locale,'Download',languageData)}
      </Button>}>
      <Workbook.Sheet data={array} name="Sheet A">
      <Workbook.Column label="Name" value="Name"/>
      <Workbook.Column label="Publisher Sensitivity" value="publisher_sen"/>
      <Workbook.Column label="Auditor Sensitivity" value="auditor_sen"/>
      </Workbook.Sheet>
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
              ...
              {
                title:  onChangeLanguage(locale,'Publisher Sensitivity',languageData),
                dataIndex: 'publisher_sen',
                key: 'publisher_sen',
                ...this.getColumnSearchProps('publisher_sen')

              },
              {
                title:  onChangeLanguage(locale,'Auditor Sensitivity',languageData),
                dataIndex: 'auditor_sen',
                key: 'auditor_sen',
                ...this.getColumnSearchProps('auditor_sen')
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Error Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
              <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Error Type',languageData)} match={match} />
                    </div>
                     <div className = "col-md-4">
                      ...
                      <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Error Type',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Error Type',languageData) : onChangeLanguage(locale,'Create Error Type',languageData)}   
          </ModalHeader>
          <ModalBody>
