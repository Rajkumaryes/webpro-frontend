...
import{errorreceivedbyService} from '../../../../../redux/pack-kpi/errorreceivedby/saga'
...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      errorreceivedbyService.fetchapi()
        .then((res) => { 
          this.setState({   
        loading : false 
  ...
  handleDelete = id => {
    errorreceivedbyService.deleteapi(id)
    .then((res) => {
      if(res){
  ...
  onChangeFileUpload(files)
    ...
    errorreceivedbyService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="Error Received By.xlsx" element={<Button  className = "button-width" color="secondary">
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Error Received By',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Error Received By',languageData)} match={match} />
                    </div>
                    <div className = "col-md-5">
                        ...
                        {onChangeLanguage(locale,' Add Error Received By',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Error Received By' : 'Create Error Received By'}   
          </ModalHeader>
          <ModalBody>
