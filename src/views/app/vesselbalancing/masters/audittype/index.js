...
class MasterPage extends Component {
      ...
      .then((res) => {
        if (res.status) {
          ...
          var regionlist = filterstatus.map(function (cusmaidid) {
            return { label: cusmaidid.name, value: cusmaidid.id.toString(), text:cusmaidid.name };
          });
          this.setState({
  ...
  onChangeFileUpload(files) {
    ...
    audittypeService.fileUpload(files[0])
      .then((res) => {
        if (res.status) {
          if (res.data.status) {
  ...
  render() {
    ...
    return (
      <>
        <title> {onChangeLanguage(locale, 'Audit Type', languageData)}</title>
        <Row>
          <Colxx xxs="12">
            <div className="row">
              <div className="col-md-8">
                <Breadcrumb heading={onChangeLanguage(locale, 'Audit Type', languageData)} match={match} />
              </div>
              ...
              <div className="col-md-4">
                ...
                <Button color="primary" className='fontstyle button-width' onClick={(event) => this.openModal()}>
                  {onChangeLanguage(locale, 'Add Audit Type', languageData)}
                </Button>
              </div>
    ...
    return (
      ...
      >
        <ModalHeader toggle={() => this.closeModal()}>
          {isEdit ? onChangeLanguage(locale, 'Update Audit Type', languageData) : onChangeLanguage(locale, 'Create Audit Type', languageData)}
        </ModalHeader>
        <ModalBody>
