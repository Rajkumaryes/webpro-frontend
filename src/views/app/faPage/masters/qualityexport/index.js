...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    qualityexportService.deletequalityexport(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Qualityexport.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    qualityexportService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Quality Export',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Quality Export',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Quality Export',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Quality Export',languageData) : onChangeLanguage(locale,'Create Quality Export',languageData)}   
          </ModalHeader>
          <ModalBody>
