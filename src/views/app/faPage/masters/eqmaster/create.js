...
import{eqmasterService} from '../../../../../redux/fa/eqmaster/saga'
...
onSubmit() { 
  const {id,name,eq,status,isEdit} = this.state;
  ...
  if( eq!=="" && name !== '')
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,eq,status)
    }
    else
    {
      this.createAPI(name,eq,status)
    }
...
createAPI(name,eq,status)
{
  this.setState({
    loading : true
  })
  eqmasterService.createeqmaster(name,eq,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,eq,status)
{
  this.setState({
    loading : true
  })
  eqmasterService.updateeqmaster(id,name,eq,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
onChangeRegion(value)
{
  ...
  render()
  {
    ...
    const {name,eq,isEdit,status,loading} = this.state
