...
class MasterPage extends Component {
 ...
 fetcharea() {
  ...
  renderTemplate()
  {
    ...
    const {data,region_data,subregiondata} = this.state
    var array = data.map(record=> {
          return {
            'name' : record.name,
            'region':getValue(region_data,'value','text',parseInt(record.region)),
            'subregion':getValue(subregiondata,'value','text',parseInt(record.subregion))
        };
      })
      return(
        
        <Workbook filename="Area.xlsx" element={
          ...
          <Workbook.Column label="Subregion" value="subregion"/>
  ...
  onChangeFileUpload(files)
    ...
    AreaService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        const {data,subregiondata,region_data} = this.state
        const columns = [
             {
              ...
              },
              {
                ...
                render: (text, record) => (	
                  <div>
                     {text !== null &&getValue(subregiondata,'value','text',parseInt(text))}
                  </div>
              ),
              filters:subregiondata,
              filterMultiple: false,
              onFilter: (value, record) => parseInt(record.subregion) === parseInt(value),
              },
              {
