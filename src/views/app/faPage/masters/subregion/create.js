...
createAPI(name,region,status)
{
  ...
  subregionService.createsubregion(name,region,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,region,status)
{
  ...
  subregionService.updatesubregion(id,name,region,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {region_data,region,name,isEdit,status,loading} = this.state
      ...
      <div>
         ...
         <Row>
            ...
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Sub Region',languageData)} 
                        value = {name}  
