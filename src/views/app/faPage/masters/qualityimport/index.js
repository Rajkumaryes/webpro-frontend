...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    qualityimportService.deletequalityimport(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Quality_import.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    qualityimportService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Quality Import',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Quality Import',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Quality Import',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Quality Import',languageData) : onChangeLanguage(locale,'Create Quality Import',languageData)}   
          </ModalHeader>
          <ModalBody>
