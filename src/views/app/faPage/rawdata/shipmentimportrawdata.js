...
import{reportService} from '../../../../redux/fa/reportrawdata/saga'
    ...
    fetchPagination(page,per_page,startdate,enddate,is_report)
    {
        ...
        if(enddate !== "" && startdate !== ""  && team_id !== "")
        {
          if ((Date.parse(enddate) > Date.parse(startdate)))
          {
            ...
            const  menu= 'FA' , 
            submenu='Shipment Import',
            type = 'Raw Data',
            audit_type= 'import'
            var date1 = moment(new Date(startdate)).format('YYYY-MM-DD HH:mm:ss a')
            var date2 = moment(new Date(enddate)).format('YYYY-MM-DD HH:mm:ss a')
            reportService.fetchapi(page,per_page,date1,date2,user_data,is_report,
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          checklist_data,area_data,importqualitydata,subregiondata,regoin_Data,region_data,
          areadata} = this.state
        var columns= [
          {
          ...
          },
          {
            ...
            render: (text, record,index) => ( 
              <div style = {{padding:'2px',width:'100px'}}>
               {getValue(regoin_Data,'value','label',text)}   
              </div>
          ), 
          },
          {
            title: onChangeLanguage(locale,'SubRegion',languageData),
            dataIndex: 'subregion',
            key: 'subregion',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
          ...
          {
            title: onChangeLanguage(locale,'Import Date',languageData),
          ...
          {
            title: onChangeLanguage(locale,'Import ID',languageData),
            dataIndex: 'import_auditor',
            key: 'import_auditor',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {text}
          ...
          {
            title: onChangeLanguage(locale,'Import Quality',languageData),
            dataIndex: 'import_quality',
            key: 'import_quality',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
             {getValue(importqualitydata,'value','label',text)}   
              </div>),           
          },
        ]
        const pagination = {
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Shipment Import Raw Data',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <Breadcrumb heading={onChangeLanguage(locale,'Shipment Import Raw Data',languageData)} match={match} />
