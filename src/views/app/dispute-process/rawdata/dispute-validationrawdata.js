...
import { reportService } from '../../../../redux/Dispute-process/dispute-validationrawdata/saga'
...
import { regionService } from '../../../../redux/Dispute-process/region/saga';
...
class QueryResolveSheet extends Component {
  ...
  componentDidMount() {
    this.fetchTeamData()
    this.fetchteam()
    this.fetchDisputestatus()
    this.fetchdisputeinput()
    this.fetchcountry()
    this.fetchregion()
    this.fetcharea()
  }
    ...
    regionService.fetchregion()
      .then((res) => {
        ...
        if (res.status) {
          ...
          this.setState({ regiondata: typelist })
        }
      })
      .catch((error) => {
        this.setState({ loading: false })
  ...
  fetchdisputeinput() {
    ...
    DisputeinputService.fetchdisputeinput()
      .then((res) => {
        this.setState({ loading: false })
        if (res.status) {
          let filterstatus = (res.data).filter(item => item.status === 1)
  ...
  render() {
    ...
    const { enddate, startdate, team_id, team_data, is_submit, loading, data, page, pageSize,
      teamdata, disputeinputdata, statusdata, countrydata, regiondata, areadata } = this.state
    var columns = [
      {
      ...
      {
        title: onChangeLanguage(locale, 'Dispute Validity', languageData),
        dataIndex: 'dispute_validity',
        key: 'dispute_validity',
        render: (text, record, index) => (
          <div style={{ padding: '2px', width: '100px' }}>
            {text}
          </div>),
      ...
      {
        title: onChangeLanguage(locale, 'Dispute Type', languageData),
      ...
      {
        title: onChangeLanguage(locale, 'Capture Date', languageData),
      ...
      {
        title: onChangeLanguage(locale, 'Followup With', languageData),
        dataIndex: 'followup_with',
        key: 'followup_with',
        render: (text, record, index) => (
          <div style={{ padding: '2px', width: '100px' }}>
            {text}
      ...
      {
        title: onChangeLanguage(locale, 'Followup With Username', languageData),
        dataIndex: 'followup_with_username',
        key: 'followup_with_username',
        render: (text, record, index) => (
          <div style={{ padding: '2px', width: '100px' }}>
            {text}
          </div>),
      ...
      {
        title: onChangeLanguage(locale, 'Charge Type', languageData),
