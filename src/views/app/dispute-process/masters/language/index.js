...
      })
      return(
        ...
        <Workbook filename="Language.xlsx" element={
          ...
          <Workbook.Column label="Language" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    LanguageService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Language',languageData),
                dataIndex: 'name',
                key: 'name',
