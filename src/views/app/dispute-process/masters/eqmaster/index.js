...
class MasterPage extends Component {
         ...
         if(res.status)   { 
            let filterstatus = (res.data).filter(item => item.status === 1)
               var typelist = filterstatus.map(function(typename) {
                    return  {text : typename.name ,value : (typename.id).toString()};
                 });  
                  this.setState({
  ...
  renderTemplate()
  {
    ...
    const {data,country_data} = this.state
    var array = data.map(record=> {
          return {
            'Module' :record.module,
            'Type' : record.type,
            'country' :record.country !== null ? getValue(country_data,'value','text',record.country):'' ,
            'EQ' : record.eq,
        };
      })
      return(
        ...
        const columns = [
              ...
              },
             {
              ...
              },
              {
                ...
                render: (text, record) => (	
                  <div>
                     {text !== null &&getValue(country_data,'value','text',text)}
                  </div>
              ),
              filters: country_data,
