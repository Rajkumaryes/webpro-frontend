...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    precollectService.deleteprecollect(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Prepaid.xlsx" element={
          ...
          <Workbook.Column label="Prepaid / Collect" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Prepaid / Collect',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Prepaid / Collect',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Prepaid',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Prepaid / Collect' : 'Create Prepaid / Collect'}   
          </ModalHeader>
          <ModalBody>
