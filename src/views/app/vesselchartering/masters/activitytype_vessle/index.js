...
class MasterPage extends Component {
  ...
  onChangeFileUpload(files)
    ...
    activitytypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="Activity Type.xlsx" element={<Button  className = "button-width" color="secondary">
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update activitytype_vessle' : 'Create Activity Type'}   
          </ModalHeader>
          <ModalBody>
