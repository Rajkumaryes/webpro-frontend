...
import{reportService} from '../../../../redux/vesselchartering/report/saga'
...
class QueryResolveSheet extends Component {
    ...
    fetchPagination(startdate,enddate,is_report)
    {
        const {} = this.state
        if(enddate !== "" && startdate !== "")
        {
          if ((Date.parse(enddate) > Date.parse(startdate)))
          {
            ...
            const menu= 'Vessel Chartering' , 
            submenu='Vessel Chartering'
            reportService.fetchapikpi(startdate,enddate,is_report,username,menu,submenu)
            .then((res) => { 
              this.setState({   
                   loading : false   
              }) 
    ...
    onSubmit(date)
    {
      ...
      if(startdate !== '')
      {
       this.fetchPagination(startdate,date,false)
      }
      else
      {
       createNotification('Please Choose Start Date','error','filled');
    ...
    getdaywiseChart(day,days_value)
  {
   ...
   console.log("lgkjgjgkj  finalDate = " , JSON.stringify(finalDate) , 'currentdate ==>' , currentdate)
    ...
    this.fetchPagination(new Date(finalDate),convertLocalToUTCDate(new Date()),false)
  }
  paginationOptions = {
    ...
    onChange: () => {
     ...
     this.fetchPagination(startdate,enddate,false)
    },
    ...
    render()
    {
        ...
        const {enddate,startdate,is_submit,loading,data} = this.state
        var columns= [
          {
            title: onChangeLanguage(locale,'Modules',languageData),
            dataIndex: 'modules',
            key: 'modules',
            render: (text) => ( 
          ...
          {
            title: onChangeLanguage(locale,'Volume',languageData),
            dataIndex: 'volume',
            key: 'volume',
            render: (text) => ( 
              <div  style = {{padding:'2px'}}>
          ...
          {
            title: onChangeLanguage(locale,'No of Error',languageData),
            dataIndex: 'no_of_Error',
            key: 'no_of_Error',
            render: (text) => ( 
              <div  style = {{padding:'2px'}}>
               {text}
            ...
            <div className = "publishuser-card-component" style = {{borderRadius:'10px',marginBottom:'30px'}}>
                          <div className = "row" style = {{padding:'10px'}}>
                             ...
                             <div className = "text-center" style = {{padding:'0px 0px 20px'}}>
                                ...
                                onClick={()=>this.fetchPagination(startdate,enddate,false)} >
                                  
                                  {onChangeLanguage(locale,'Generate Report',languageData)}</Button> 
                               <Button className = "button-width" color="secondary" 
                                onClick={()=>this.fetchPagination(startdate,enddate,true)} >
