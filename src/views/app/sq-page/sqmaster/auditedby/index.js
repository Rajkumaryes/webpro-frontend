...
import{AuditedbyService} from '../../../../../redux/sq/sqmasters/auditedby/saga'
...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      AuditedbyService.fetchauditedby()
        .then((res) => { 
          this.setState({   
        loading : false 
  ...
  handleDelete = id => {
    AuditedbyService.deleteauditedby(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Audited By.xlsx" element={
          ...
          <Workbook.Column label="Audited By" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    AuditedbyService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Audited By',languageData),
                dataIndex: 'name',
                key: 'name',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Audited By',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Audited By',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Audited By',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Audited By' : 'Create Audited By'}   
          </ModalHeader>
          <ModalBody>
