...
import {onChangeLanguage,getTimeDifference,getValue,getAreaValue,getEnableStatus,
      customStyles,getStausOptionEnable,
      convertLocalToUTCDate,convertUTCToLocalDate,getStausOptionAdminEnable} from '../../../../helper'
...
class QueryResolveSheet extends Component {
      ...
      publisher_status,auditor,auditor_status,start_date,end_date,region,is_create)
    {
      ...
      quotationdetailsService.filtercontrolapi(page,per_page,request_no,area,priority,size,quotation_no,publisher,publisher_status,auditor,auditor_status,start_date,end_date,region)
        .then((res) => { 
          this.setState({   
            loading : false,
            select_index : -1,
    ...
    { 
      ...
      const index = data.findIndex(x => x.id === id)
        if(index >= 0)
        {
            ...
            if(key === 'publisher')
            {
              if(data[index].publisher_status === null || data[index].publisher_status === '')
                {
                  var list = status_data.filter(option =>(option.label).toUpperCase() === 'TO BE STARTED')

                  if(list.length > 0)
                  {
                    data[index].publisher_status = list[0].value
                  }
  ...
  renderTemplate()
    {
      ...
      const column_name = ["Request No","Reqno","Quotationno","Typeofrequest","Region","Area","Priority","Size",
                           ...
                           "Contractcomplete-Pub","Contractcomplete-Aud"]
        return(
