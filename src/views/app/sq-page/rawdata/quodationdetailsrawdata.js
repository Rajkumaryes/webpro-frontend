    ...
    fetchPagination(page,per_page,startdate,enddate,is_report)
    {
        ...
        if(enddate !== "" && startdate !== ""  && team_id !== "")
        {
          if ((Date.parse(enddate) > Date.parse(startdate)))
          {
            ...
            const report_url =  'quotationdetails',
            menu= 'SQ' , 
            submenu='Quotation Details',
            type = 'Raw Data'
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
            size_data,area_data,region_data,priority_data,status_data,typeofrequestdata} = this.state
        var columns = [
            {
             ...
             },
             {
               ...
               render: (text, record) => (	
                <div>
                    {getValue(typeofrequestdata,'value','label',text)} 
                </div>
            ),  
        },
        {
          ...
          },
          {
            ...
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
                {(text !=='' && text !== null)?moment(text).format('MM/DD/YYYY hh:mm:ss a'):''} 
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Quotation Details Raw Data',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <Breadcrumb heading={onChangeLanguage(locale,'Quotation Details Raw Data',languageData)} match={match} />
