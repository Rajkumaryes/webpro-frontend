...
class QueryResolveSheet extends Component {
    ...
    fetchData(id) {  
        ...
        quotationdetailsService.fetchIndividualapi(id)
          .then((res) => { 
            this.setState({   
          loading : false 
     ...
     {
       var date = ''
       if(dates !== 'Invalid date' && dates !== '' && dates !== null)
       {
        date = moment(convertLocalToUTCDate(dates)).format('MM/DD/YYYY hh:mm:ss a')
       }
     ...
     setValue(record)
     {
         if(record && record !== null)
         {
             this.setState({
                ...
                customer_require: this.getproperDate(record.customer_require),
          ...
          if(res.status)
            {
              if(res.data && res.data !== null)
              {
                  ...
                  {
                    const value = this.getShared_no_of_rutes(res.data)
                    if(value !== null && value !== '')
                    {
                      this.setState({
    ...
    getTimebasedonstatus(id,key)
    {
      ...
      if((value).toUpperCase() === 'TO BE STARTED')
      {
        if(key === 'publisher_status')
        {
          this.setState({
            ...
            pub_routes:'',
            auditor:'',
            auditor_status:auditor_statuss,
            aud_in_progress : '',
            aud_pending_in:'',
            aud_pending_out:'',
            aud_done_time:'',
      ...
      else if((value).toUpperCase() === 'DONE')
      {
        ...
        this.setState({
         ...
         this.fetchDatabasedReq(this.state.id)
      ...
      onClickerrorlog()
      {
        ...
        this.props.history.push(`/app/sq/correction-log`)
      }
      onClickCurrenrecord(id)
      {
      
        localStorage.setItem('request_no_id',id)
        this.props.history.goBack()
      }
  ...
  onSubmit()
  {
    const{id,request_no,quotation_no,type_of_req,priority,size,area,region
        ...
        aud_done_time,aud_disregrads,aud_routes,comments_audit,start_time,end_time,updated_start_time,customer_require}=this.state
      var isfill = true
      ...
      if(isfill === true)
      {
        ...
        quotationdetailsService.updateapi(id,request_no,quotation_no,type_of_req,priority,size
          ,received_time,publisher,publisher_status,pub_in_progress,pub_pending_in,pub_pending_out,
          pub_done_time,pub_disregrads,pub_total,pub_routes,comments_publisher,auditor,
          auditor_status,aud_in_progress,aud_pending_in,aud_pending_out,error_log,
          aud_done_time,aud_disregrads,aud_total,aud_routes,comments_audit,start_time,
          end_time,updated_start_time,updated_end_time,region,area,username,customer_require)
          .then((res) => { 
            this.setState({   
              loading : false,
              is_changed_staus:false   
  ...
  handleDelete(id)
  {
    ...
    quotationdetailsService.deleteapi(id)
    .then((res) => {
      if(res){
        this.setState({   
            loading : false     
  ...
  getDisabled(key,data,value)
  {
    const {request_no,publisher,publisher_status,auditor} = this.state
    const {role_permission_data} = this.props
    const is_adimin_Edit = getPermission(role_permission_data,'SQ','Details Box',true)
    var isDisabled = true
  ...
  setDate(date)
  {
    ...
    render()
    { 
        const{loading,id,user_data,size_data,priority_data,status_data,request_no,quotation_no,amd_no,priority,size
        ,received_time,publisher,publisher_status,pub_in_progress,pub_pending_in,pub_pending_out,
        pub_done_time,pub_disregrads,pub_total,pub_routes,comments_publisher,auditor,
        auditor_status,aud_in_progress,aud_pending_in,aud_pending_out,error_log,
        aud_done_time,aud_disregrads,aud_total,aud_routes,comments_audit,is_adimin_delete}=this.state
        const {locale,languageData,role_permission_data,match,is_submit} = this.props
        const is_adimin_Edit = getPermission(role_permission_data,'SQ','Details Box',true)
        return (
            <>
            <title>{onChangeLanguage(locale,'General Information',languageData)}</title>
            {loading && 
              <div>
            ...
            <Row>
              ...
              <div className = "row">
                      ...
                      <Breadcrumb heading={onChangeLanguage(locale,'General Information',languageData)} match={match} />
                    </div>
                  </div>
            ...
            <div className = "row" style = {{}}>
                    ...
                    <div className = "col-md-2 space-margin"  >
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Quote No',languageData)}<a style = {{color :'red'}}>*</a></Label>
                            <Input  className = {is_submit === true && quotation_no === ''?  "error-border":"fontstyle text-background" } 
                            placeholder = ''
                            value = {quotation_no}  
                        ...
                        <div className = "col-md-2 space-margin"  >
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Received Time',languageData)}<a style = {{color :'red'}}>*</a>
                        ...
                        <div className = "col-md-2 space-margin"  >
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Priority',languageData)}<a style = {{color :'red'}}>*</a></Label>
                            <Select className = {is_submit === true && priority === ''?  "error-border-select":"react-select fontstyle" }  
                        ...
                        <div className = "col-md-2 space-margin"  >
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Size',languageData)}<a style = {{color :'red'}}>*</a></Label>
                            <Select className = {is_submit === true && size === ''?  "error-border-select":"react-select fontstyle" }  
                                classNamePrefix="react-select"
                                name="form-field-name"
                    ...
                    <div className = "col-lg-2-0 space-margin">
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Status',languageData)}</Label>
                             <Select className="react-select fontstyle"
                                ...
                                styles={customStyles}
                                isDisabled={this.getDisabled('publisher_status',status_data,publisher_status)}
                ...
                <div className = "publishuser-card-component" style = {{borderRadius:'10px',marginBottom:'30px'}}>
                    ...
                    <div className = "col-lg-2-0 space-margin">
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Audited By',languageData)}</Label>
                             <Select  className="react-select fontstyle"
                                ...
                                options={user_data.filter(option =>option.value !== publisher || publisher === 'SHAREDR')}
                    ...
                    <div className = "col-lg-2-0 space-margin">
                        ...
                        <div className = "col-md-2" style = {{marginTop:'10px'}}>
                                    ...
                                    checked = {error_log}   onChange= {(e)=> this.setState({error_log : e.target.checked})} /> 
                                    {onChangeLanguage(locale,'Error Log',languageData)}      
                                    </a> 
                            </div>
                        <div className = "col-md-12 space-margin">
                            <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Comments',languageData)}</Label>
                      ...
                      {onChangeLanguage(locale,'Save',languageData)}  
                       ...
                       {onChangeLanguage(locale,'Back',languageData)} 
                        ...
                        <Button 
                            ...
                            onClick={()=>this.onClickCurrenrecord(id)}
                        >
    ...
    getShared_no_of_rutes(shared_data)
    {
      const {is_publish,publisher,auditor} = this.state
      var value = ''
    ...
    setPendingtime()
    {
      const {pending_out,is_publish,pub_pending_in,aud_pending_in,status_data} = this.state
