          ...
          if ((Date.parse(enddate) > Date.parse(startdate)))
           {
            ...
            const report_url =  'unassignedlist',
  ...
  getamendmentvalue(id,array)
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          issuecode_data,exception_type_data,queue_name_data,amendment_due_to_data,process_type_data,reason_data,assigned_to_data,cargo_type_data,} = this.state
        var columns= [
          {
          ...
          },
          {
            ...
            {
                title:onChangeLanguage(locale,'Queue Name',languageData) ,
                dataIndex: 'queuename',
                key: 'queuename',
                render: (text, record) => ( 
                  <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
                    {getValue(queue_name_data, 'value', 'label', text)} 
                  </div>),
              },
              {
                ...
                render: (text, record) => ( 
                  <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
                    {getValue(process_type_data, 'value', 'label', text)} 
                  </div>),
              },
              {
