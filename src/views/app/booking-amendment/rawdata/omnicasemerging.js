      ...
      statusofthecaseMergingService.fetchapi()
        .then((res) => {
          ...
          if (res.status) {
            ...
            this.setState({
                statusofthecase_merging_data: regionlist,
            })
          }
        })
        .catch((error) => { this.setState({ loading: false }) });
          ...
          if ((Date.parse(enddate) > Date.parse(startdate)))
           {
            ...
            const report_url =  'omnicasemerging',
  ...
  getamendmentvalue(id,array)
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          issuecode_data,exception_type_data,query_type_data,amendment_due_to_data,statusofthecase_merging_data,reason_data,assigned_to_data,cargo_type_data,} = this.state
        var columns= [
          {
          ...
          },
          {
            ...
            {
                title:onChangeLanguage(locale,'Query Type',languageData) ,
                dataIndex: 'query_type',
                key: 'query_type',
                render: (text, record) => ( 
                  <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
                    {getValue(query_type_data, 'value', 'label', text)} 
                  </div>),
              },
              {
                ...
                render: (text, record) => ( 
                  <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
                    {getValue(statusofthecase_merging_data, 'value', 'label', text)} 
                  </div>),
              },
              {
