          ...
          if ((Date.parse(enddate) > Date.parse(startdate)))
           {
            ...
            const report_url =  'errorlogissue',
            menu= 'Booking Amendment' , 
            submenu='Booking Amendment',
            type = 'Raw Data'
            if(is_report === true){
                reportService.fetchapidownload(report_url,page,per_page,startdate,enddate,user_data,is_report,
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          issuecode_data,exception_type_data,error_category_data,amendment_due_to_data,process_type_data,reason_data,assigned_to_data,cargo_type_data,} = this.state
        var columns= [
          {
            title: onChangeLanguage(locale,'GCC User Id',languageData),
            dataIndex: 'user_id',
            key: 'user_id',
          ...
          },
          {
            ...
            render: (text, record) => (
              <div className="row d-flex justify-content-center" style={{ padding: '2px', width: '200px' }}>
                {text !== '' ? moment(text).utc().format('MM/DD/YYYY HH:mm:ss a') : ''}
              </div>
            ),
          },
          {
            title: onChangeLanguage(locale,'Error Reviewed UserID',languageData),
          ...
          },
          {
            ...
            render: (text, record) => (
              <div className="row d-flex justify-content-center" style={{ padding: '2px', width: '200px' }}>
                {text == 1 ? 'Accepted' : text == 2 ? 'Refuted' : 'Learning'}
              </div>
            ),
          },
          {
            title: onChangeLanguage(locale,'Error UserID',languageData),
          ...
          {
            title: onChangeLanguage(locale,'Error Received From',languageData),
            dataIndex: 'error_received_from',
            key: 'error_received_from',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'200px'}}>
                {text}
              </div>)
            ...
            render: (text, record,index) => ( 
                <div  style = {{padding:'2px',width:'200px'}}>
                {text == 1 ? "Internal" : "External"}
                </div>)
            },
            {
                title: onChangeLanguage(locale,'Area Comments',languageData),
            ...
            {
                title: onChangeLanguage(locale,'GCC Comments',languageData),
                dataIndex: 'gcc_comments',
                key: 'gcc_comments',
                render: (text, record,index) => ( 
                    <div  style = {{padding:'2px',width:'200px'}}>
                    {text}
