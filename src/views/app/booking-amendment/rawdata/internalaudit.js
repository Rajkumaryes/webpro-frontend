          ...
          if ((Date.parse(enddate) > Date.parse(startdate)))
           {
            ...
            const report_url =  'internalaudit',
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          issuecode_data,exception_type_data,audit_type_data,amendment_due_to_data,process_type_data,reason_data,assigned_to_data,cargo_type_data,} = this.state
        var columns= [
          {
          ...
          },
          {
            ...
            render: (text, record) => (
              ...
              {
                title:onChangeLanguage(locale,'Audit Type',languageData) ,
                dataIndex: 'audittype',
                key: 'audittype',
                render: (text, record) => ( 
                  <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
                    {getValue(audit_type_data, 'value', 'label', text)} 
                  </div>),
              },
              {
