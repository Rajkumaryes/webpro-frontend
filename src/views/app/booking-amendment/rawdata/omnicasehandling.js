        ...
        .then((res) => {
          ...
          if (res.status) {
            ...
            var regionlist = filterstatus.map(function (cusmaidid) {
              return {
                label: cusmaidid.exception_reason ,value: cusmaidid.id.toString()
              };
            });
      ...
      amendmentduetoService.fetchapi()
        .then((res) => {
          ...
          if (res.status) {
            ...
            this.setState({
                amendment_due_to_data: regionlist,
            })
          }
        })
        .catch((error) => { this.setState({ loading: false }) });
  ...
  getamendmentvalue(id,array)
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          issuecode_data,exception_type_data,amendment_type_data,amendment_due_to_data,amendment_status_data,reason_data,assigned_to_data,cargo_type_data,} = this.state
        var columns= [
          {
          ...
          {
            title: onChangeLanguage(locale,'No of Booking Number',languageData),
            dataIndex: 'no_of_booking_number',
            key: 'no_of_booking_number',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'200px'}}>
                {text}
              </div>)
            ...
            {
              title: onChangeLanguage(locale,'Amendment Caused USER ID',languageData),
              dataIndex: 'amendment_caused_userid',
              key: 'amendment_caused_userid',
              render: (text, record) => ( 
                  <div className="row d-flex justify-content-center"  style = {{padding:'2px',width:'200px'}}>
                    {text}
                  </div>)
              ...
              },
            {
                ...
                render: (text, record) => ( 
                  <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
                    {getValue(amendment_type_data, 'value', 'label', text)} 
                  </div>),
              },
              {
              ...
              //   render: (text, record) => ( 
              //     <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
              //      { record.exception_reason && record.exception_reason.map((value,index) => 
              //       <div >
              //           {/* {index} */}
              //          {this.getamendmentvalue(value,this.state.exception_type_data)}
              //       </div>)} 
            ...
            //   },
              {
                title:onChangeLanguage(locale,'Amendment Due To',languageData) ,
                dataIndex: 'amendment_dueto',
                key: 'amendment_dueto',
                render: (text, record) => ( 
                  <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
                    {getValue(amendment_due_to_data, 'value', 'label', text)} 
                  </div>),
              },
              {
