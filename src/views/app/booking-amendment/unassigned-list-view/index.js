...
import { unassignedlistService } from '../../../../redux/bookingamendment/unassignedlistview/saga'
...
class BookingSheet extends Component {
  ...
  componentDidMount() {
    ...
    this.fetchissie()
    this.fetchqueuename()
    this.fetchprocesstype()
    this.fetchunassignedlistCount()
  }
  fetchunassignedlistCount(){
    this.setState({loading:true})
    const {username} = this.props
    unassignedlistService.fetchunassignedlistcount(username)
    .then((res) => {
       if(res.status)   { 
              let filterstatus = res.data;
              let lastdata = res.lastcount; 
              this.setState({ 
                unassignedlistcount:filterstatus, 
                unassignedlistcountlast:lastdata     
              }) 
             }
             else{
  ...
  onSubmit() {
    const { id, region, area, team,
      updated_start_time, received_date,queuename,processtype,crm_case_no,booking_number } = this.state;
    var is_fill = false

    if (region !== '' && area !== '' && team !== '' && received_date !== '' && queuename !== '' && processtype !== '' ) {
      this.setState({
        crmsearch: true
      })
      is_fill = true
    ...
    if (is_fill === true) {
      ...
      unassignedlistService.createbookingamendment(id, region, area, team, username, queuename, processtype,crm_case_no,booking_number,
  ...
  //       return name
  // } 
  handlequeuename = (selectedOptions) => {
    this.setState({ queuename: selectedOptions.value })
  }
  handleprocesstype = (selectedOptions) => {
    this.setState({ processtype: selectedOptions.value })
  }
  onChangeFileUpload(files)
    ...
    unassignedlistService.fileUpload(files[0])
      .then((res) => { 
        this.setState({
          loading : false
        })
  ...
  renderTemplate()
  {
    ...
    const column_name = ["GCC User ID","Received Date", "Booking Number","CSB Office","CRM Case Number","Queue Name","Process Type","Start Date","End Date"]
      return(
        
        <Workbook filename="Unassignedlist.xlsx" element={
  ...
  render() {
    ...
    const { loading, region, area, team, csb_office, assigned_to,
      cargo_type, received_date, queuename, processtype, queuename_data, processtype_data,unassignedlistcount,unassignedlistcountlast,crm_case_no,booking_number,
    ...
    return (
      ...
      <>
        ...
        <title>{onChangeLanguage(locale, 'Unassigned List View', languageData)}</title>
        <Row>
          <Colxx xxs="12">
            <div className="row">
              <div className="col-md-5">
                <Breadcrumb heading={onChangeLanguage(locale, 'Unassigned List View', languageData)} match={match} />
              </div>
              ...
              <div className = "col-md-2">
                        <h2 style = {{marginTop:'15px'}}>Total EQ : {unassignedlistcount}</h2>
                    </div>
                    <div className = "col-md-2">
                        <h2  style = {{marginTop:'15px'}}>Last EQ : {unassignedlistcountlast}</h2>
                    </div>
        ...
        <div className="publishuser-card-component" style={{ padding: '10px', borderRadius: '10px', marginBottom: '30px' }}>
          <div className="row" style={{ marginBottom: '30px' }}>
            ...
            <div className="col-md-3 space-margin">
              <Label className="fontstyle normal-font" >{onChangeLanguage(locale, 'Queue Name', languageData)}<a style={{ color: 'red' }}>*</a></Label>
              <Select
                className={is_submit === true && queuename === '' ? "error-border-select" : "react-select fontstyle"}
                classNamePrefix="react-select"
                name="form-field-name"
                value={queuename_data.filter(option => option.value === queuename)}
                options={queuename_data}
                onChange={this.handlequeuename}
              />
            </div>
            <div className="col-md-3 space-margin">
            ...
            <div className="col-md-3 space-margin">
              ...
              <Select
                className={is_submit === true && processtype === '' ? "error-border-select" : "react-select fontstyle"}
                classNamePrefix="react-select"
                name="form-field-name"
                value={processtype_data.filter(option => option.value === processtype)}
                options={processtype_data}
                onChange={this.handleprocesstype}
              />
            </div>
