...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    processtypeService.deleteapi(id)
    .then((res) => {
      if(res){
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Process Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Process Type',languageData)} match={match} />
                    </div>
                    ...
                    {/* <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Process Type',languageData)}
                      </Button>
                    </div>
                  </div>
    ...
    renderModal()
  {
    ...
    return (
        ...
        >
          ...
          {isEdit ? onChangeLanguage(locale,'Update Process Type' , languageData) : onChangeLanguage(locale,'Create Process Type' , languageData) }     
         </ModalHeader>
          <ModalBody>
