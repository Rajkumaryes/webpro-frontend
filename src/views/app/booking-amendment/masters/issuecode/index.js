...
          return {
            ...
            'area' : record.area,
            'geocode':record.geo_code

        };
      })
      return(
        <Workbook filename="Issuecode.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    issuecodeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Issuer Code',languageData),
                dataIndex: 'issue_code',
                key: 'issue_code',
                ...this.getColumnSearchProps('issue_code')
              },
              {
                title:  onChangeLanguage(locale,'Geocode',languageData),
                dataIndex: 'geo_code',
                key: 'geo_code',
                ...this.getColumnSearchProps('geo_code')
              },
              {
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Issuer Code',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Issuer Code',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Issuer Code',languageData)}
                      </Button>
                    </div>
                  </div>
    ...
    renderModal()
  {
    ...
    return (
        ...
        >
          ...
          {isEdit ? onChangeLanguage(locale,'Update Issuer Code' , languageData) : onChangeLanguage(locale,'Create Issuer Code' , languageData) }     
         </ModalHeader>
          <ModalBody>
