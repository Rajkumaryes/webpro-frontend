...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    exceptiontypeService.deleteapi(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Container Type.xlsx" element={
          ...
          <Workbook.Column label="Exception Type" value="exception_type"/>
          <Workbook.Column label="Exception Reason" value="exception_reason"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Exception Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Exception Type',languageData)} match={match} />
                    </div>
                    ...
                    {/* <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Exception Type',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Exception Type',languageData) : onChangeLanguage(locale,'Create Exception Type',languageData)}   
          </ModalHeader>
          <ModalBody>
