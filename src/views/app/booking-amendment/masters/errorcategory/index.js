...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    errorcategoryService.deleteapi(id)
    .then((res) => {
      if(res){
...
      })
  ...
  onChangeFileUpload(files)
    ...
    amendmenttypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
              ...
              {
                title:  onChangeLanguage(locale,'Gravity',languageData),
                dataIndex: 'gravity',
                key: 'gravity',
              },
              {
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Error Category',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Error Category',languageData)} match={match} />
                    </div>
                    ...
                    {/* <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Error Category',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Error Category',languageData) : onChangeLanguage(locale,'Create Error Category',languageData)}   
          </ModalHeader>
          <ModalBody>
