...
import { errorcategoryService } from '../../../../redux/bookingamendment/errorcategory/saga'
import { processtypeService } from '../../../../redux/bookingamendment/processtype/saga'
import { errorlogissueService } from '../../../../redux/bookingamendment/errorlogissue/saga'
...
class BookingSheet extends Component {
  ...
  onSubmit() {
    const { id, region, area, team,
      updated_start_time, received_date,error_reported_date, booking_number,crm_case_no,error_userid,error_received_from,error_type,email_subject,error_reviewed_userid, error_status, error_category, area_comments,gcc_comments } = this.state;
    var is_fill = false

    if (region !== '' && area !== '' && team !== '' && received_date !== ''  && error_category !== '' && error_userid !== '' && error_status !== ''
    && error_reviewed_userid !== '' && error_reported_date !== '' && email_subject !== '' && error_received_from !== '' && error_type !== '' && crm_case_no !== '') {
      this.setState({
        crmsearch: true
      })
      is_fill = true
    ...
    if (is_fill === true) {
      ...
      let end_date = convertLocalToUTCDate(new Date()),
        ...
        error_reported_dates = convertLocalToUTCDate(error_reported_date)

        errorlogissueService.createbookingamendment(id, region, area, team, username, error_status, error_category, booking_number,crm_case_no,error_reviewed_userid,error_reported_dates,
          error_userid,error_received_from,error_type,email_subject, area_comments,gcc_comments,
  ...
  setValue(record, id) {
    ...
    if (record !== null && record) {
      this.setState({
        ...
        error_reviewed_userid: record.error_reviewed_userid,
        error_userid: record.error_userid,
        error_received_from:record.error_received_from,
  ...
  async onPasteS9610() {
    clipboard.readText().then((text) => {
      ...
      var error_reported_date = record.received_date + ' ' + record.received_time;
      this.setState({
        csb_office: record.csb_office,
        last_pod: record.last_pod,
        end_pod: record.end_pod,
        ...
        error_reported_date: this.validDate(error_reported_date, 'Received Date'),
        region: getValue(this.state.issuecode_data, 'issue_code', 'id', value),
        area: getValue(this.state.issuecode_data, 'issue_code', 'id', value),
        team: getValue(this.state.issuecode_data, 'issue_code', 'id', value)
  ...
  onChangeFileUpload(files)
    ...
    errorlogissueService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    const column_name = ["GCC User ID","Received Date", "Booking Number","CSB Office","CRM Case Number","Error Reviewed UserID","Error Reported Date", "Error Status", "Error UserID",
    "Email Subject", "Error Received From","Error Category","Error Type","Area Comments","GCC Comments","Start Date","End Date"]
      return(
        
        <Workbook filename="ErrorLogIssues.xlsx" element={
  ...
  //       return name
  // } 
  handleerror_status = (selectedOptions) => {
    this.setState({ error_status: selectedOptions.value })
  }
  handleerror_type = (selectedOptions) => {
    this.setState({ error_type: selectedOptions.value })
  }
  handleerrorcategory = (selectedOptions) => {
    this.setState({ error_category: selectedOptions.value })
  }
  render() {
    const { match, locale, languageData, username } = this.props
    const { loading, region, area, team, csb_office, assigned_to,
      cargo_type, received_date, error_status, error_category,errorcategory_data, area_comments,gcc_comments, booking_number,crm_case_no,error_reviewed_userid,error_userid,email_subject,
      error_received_from,error_reported_date,error_type,
      last_pod, end_pod, start_date, end_date, is_search, is_submit, searchdata, teamdata, tabledata, tableindex, crmsearch, } = this.state
    var error_status_data = [
      {
        "label": "Accepted",
        "value": "1"
      },
    ...
    return (
      ...
      <>
        ...
        <title>{onChangeLanguage(locale, 'Error Log Issue', languageData)}</title>
        <Row>
          <Colxx xxs="12">
            <div className="row">
              <div className="col-md-8">
                <Breadcrumb heading={onChangeLanguage(locale, 'Error Log Issue', languageData)} match={match} />
              </div>
        ...
        <div className="publishuser-card-component" style={{ padding: '10px', borderRadius: '10px', marginBottom: '30px' }}>
          <div className="row" style={{ marginBottom: '30px' }}>
            ...
            <div className="col-md-3 space-margin">
              ...
              <Input className={is_submit === true && error_reviewed_userid === '' ? "error-border-paste" : "fontstyle text-background-paste"}
            ...
            <div className="col-md-3 space-margin">
              <Label className="fontstyle normal-font" >{onChangeLanguage(locale, 'Error Reported Date', languageData)}<a style={{ color: 'red' }}>*</a></Label>
              <a className='fontstyle mandatory-label'>
                {is_submit === true && error_reported_date === '' && `${onChangeLanguage(locale, 'Mandatory Field', languageData)}`}</a>
              <DatePicker
                selected={error_reported_date}
                className="text-background-paste"
                onChange={(date) => this.setState({ error_reported_date: date })}
              />
            </div>
            ...
            <div className="col-md-3 space-margin">
              ...
              <Select
                ...
                onChange={this.handleerror_status}
              />
            </div>
            <div className="col-md-3 space-margin">
            ...
            <div className="col-md-3 space-margin">
              ...
              <Input className={is_submit === true && email_subject === '' ? "error-border-paste" : "fontstyle text-background-paste"}
                value={email_subject}
                disabled={is_search === true ? true : false}
            ...
            <div className="col-md-3 space-margin">
              ...
              <Input className={is_submit === true && error_received_from === '' ? "error-border-paste" : "fontstyle text-background-paste"}
            ...
            <div className="col-md-3 space-margin">
              ...
              <Select
                className={is_submit === true && error_category === '' ? "error-border-select" : "react-select fontstyle"}
                classNamePrefix="react-select"
                name="form-field-name"
                value={errorcategory_data.filter(option => option.value === error_category)}
                options={errorcategory_data}
            ...
            <div className="col-md-3 space-margin">
              ...
              <Select
                ...
                onChange={this.handleerror_type}
              />
            </div>
            <div className="col-md-3 space-margin">
              <Label className="fontstyle normal-font" >{onChangeLanguage(locale, 'Area Comments', languageData)}</Label>
              <textarea className={"fontstyle textarea-background"}
                value={area_comments}
                onChange={(e) => this.setState({ area_comments: e.target.value })}
              />
            </div>
            <div className="col-md-3 space-margin">
              <Label className="fontstyle normal-font" >{onChangeLanguage(locale, 'GCC Comments', languageData)}</Label>
              <textarea className={"fontstyle textarea-background"}
                value={gcc_comments}
                onChange={(e) => this.setState({ gcc_comments: e.target.value })}
              />
            </div>
          </div>
