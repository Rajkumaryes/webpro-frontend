...
saveCustomDateConfig = (enabled, applydays) => {
    const { recordId } = this.state;
    this.setState({ isSubmitting: true });

    // Call update if id exists, otherwise create
    const savePromise = recordId
        ? customdateService.applydateupdateapi(recordId, enabled, applydays) // backend update
        : customdateService.applydatecreateapi(enabled, applydays);          // backend create
...
handleApplyDateToggle = (checked) => {
    const { howManyDays, recordId } = this.state;

    this.setState({
        applyDateEnabled: checked,
    ...
    if (!checked && recordId) {
        // Delete the row
        customdateService.applydateupdateapi(recordId, false, 0)
            .then((res) => {
                if (res.status) {
  ...
  handleSaveConfiguration = () => {
    const { howManyDays, validationError, applyDateEnabled } = this.state;

    if (!applyDateEnabled) {
      createNotification('Please enable Apply Date first', 'error', 'filled');
      return;
    }
    if (validationError) {
  ...
  renderCustomDateSection() {
    const { applyDateEnabled, howManyDays, validationError, isSubmitting, locale, languageData } = this.state;
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '24px' }}>
          <h3 style={{ color: '#121C50', marginBottom: '8px', fontSize: '20px', fontWeight: '600' }}>
            {onChangeLanguage(locale, 'Apply Date Configuration', languageData)}
          </h3>
        ...
        <div style={{
          display: 'grid',
          gridTemplateColumns: applyDateEnabled ? '1fr 280px 120px' : '1fr',
          gap: '20px',
          marginBottom: '20px'
        }}>
          {/* Toggle */}
          <div style={{
            ...
            padding: '20px 24px',
            backgroundColor: applyDateEnabled ? '#fef7f0' : '#f8fafc',
            border: `2px solid ${applyDateEnabled ? 'green' : '#e2e8f0'}`,
            borderRadius: '10px'
          }}>
            <div>
              <Label style={{
                fontWeight: '600',
                color: applyDateEnabled ? '#121C50' : '#64748b',
                fontSize: '16px',
                marginBottom: '6px',
                display: 'block'
              }}>
                {onChangeLanguage(locale, 'Enable Apply Date', languageData)}
              </Label>
              <div style={{
                color: applyDateEnabled ? 'green' : '#94a3b8',
                fontSize: '13px',
                fontWeight: '500'
              }}>
                {applyDateEnabled ? 'Apply date is active' : 'Click to enable apply date'}
              </div>
            </div>
            <Switch
              checked={applyDateEnabled}
              onChange={this.handleApplyDateToggle}
              disabled={isSubmitting}
              style={{ backgroundColor: applyDateEnabled ? 'green' : undefined }}
            />
          </div>
  ...
  render() {
    ...
    return (
      <>
        <title>{onChangeLanguage(locale, 'Apply Date', languageData)}</title>
        <Row>
          <Colxx xxs="12">
            <Breadcrumb heading={onChangeLanguage(locale, 'Apply Date', languageData)} match={match} />
