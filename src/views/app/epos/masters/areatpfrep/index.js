...
class MasterPage extends Component {
       ...
       if(res.status)   { 
          let filterstatus = (res.data).filter(item => item.status === 1)
             var regionlist = filterstatus.map(function(response) {
                  return  {label : response.region ,value : response.region};
               });  
                this.setState({
                regoindata :  regionlist,
                })
             }
             else{
  ...
  handleDelete = id => {
    areatpfrepService.deletearearegion   (id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Region.xlsx" element={
          ...
          <Workbook.Column label="Region" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    areatpfrepService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        const {data,regoindata} = this.state
        const columns = [
             {
