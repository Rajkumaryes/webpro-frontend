...
createAPI(name,area,status)
{
  ...
  stturncodeService.createstturncode(name,area,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,area,status)
{
  ...
  stturncodeService.updatestturncode(id,name,area,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
      ...
      <div>
        ...
        <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label> {onChangeLanguage(locale,'Stturn',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Stturn',languageData)} 
                        value = {name}  
