         ...
         if(res.status)   { 
            let filterstatus = (res.data).filter(item => item.status === 1)
               var list = filterstatus.map(function(areaaval) {
                    return  {text : areaaval.name ,value : (areaaval.id).toString()};
                 });  
                  this.setState({
...
          return {
            ...
            'Name' : record.module !== 'Street Turns' && record.name !== null ? this.getacitivity(record.module,record.name):record.name ,
            'EQ' : record.eq,
        };
      })
      return(
        ...
        const columns = [
             {
              ...
              },
              {
                ...
                render: (text, record) => (	
                  <div>
                    {record.module === 'Street Turns' && text !== null && text}
                    {record.module !== 'Street Turns' && this.getacitivity(record.module,text)  } 
                  </div>
              ),
