...
      })
  ...
  onChangeFileUpload(files)
    ...
    hvsareaService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'HVS Area',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'HVS Area',languageData)} match={match} />
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update HVS Area',languageData) : onChangeLanguage(locale,'Create HVS Area',languageData)}   
          </ModalHeader>
          <ModalBody>
