...
      })
  ...
  onChangeFileUpload(files)
    ...
    bpcareaService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'BPC Area',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'BPC Area',languageData)} match={match} />
                    </div>
                    ...
                    {/* <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add BPC Area',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update BPC Area',languageData) : onChangeLanguage(locale,'Create BPC Area',languageData)}   
          </ModalHeader>
          <ModalBody>
