...
import{dectypeService} from '../../../../redux/projectmasters/dectype/saga';
class MasterPage extends Component {
  ...
  handleDelete = id => {
    dectypeService.deletedectype(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Data.xlsx" element={
          ...
          <Workbook.Column label="Dectype" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    dectypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Dec Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Dec Type',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Dec Type',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Dec Type' : 'Create Dec Type'}   
          </ModalHeader>
          <ModalBody>
