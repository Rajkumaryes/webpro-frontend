...
createAPI(name,status)
{
  ...
  tenderfeedbackService.createtenderfeedback(name,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,status)
{
  ...
  tenderfeedbackService.updatetenderfeedback(id,name,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
            ...
            <Loading loading={true} background="rgba(239,100,50, 0.1)" loaderColor="#EF6432" />
             ...
             <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label> {onChangeLanguage(locale,'Feedback',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
               
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Feedback',languageData)} 
                        value = {name}  
