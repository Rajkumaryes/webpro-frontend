...
import{reportbyService} from '../../../../redux/projectmasters/reportby/saga'
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    reportbyService.deletereportby(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Reportedby.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    reportbyService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Reported By',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Reported By',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Reportedby',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Reported By' : 'Create Reported By'}   
          </ModalHeader>
          <ModalBody>
