...
import{buddyuseridService} from '../../../../redux/projectmasters/buddyuserid/saga'
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      buddyuseridService.fetchbuddyuserid()
        .then((res) => { 
          this.setState({   
        loading : false 
  ...
  handleDelete = id => {
    buddyuseridService.deletebuddyuserid(id)
    .then((res) => {
      if(res){
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Buddy UserId',languageData),
                dataIndex: 'name',
                key: 'name',
              },
              {
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Buddy UserId',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-10">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Buddy UserId',languageData)} match={match} />
                    </div>
                    <div className = "col-md-2">
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Buddy UserId',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Buddy UserId' : 'Create Buddy UserId'}   
          </ModalHeader>
          <ModalBody>
