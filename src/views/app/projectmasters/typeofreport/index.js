...
import{typeofreportService} from '../../../../redux/projectmasters/typeofreport/saga';
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    typeofreportService.deletetypeofreport(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Typeofreport.xlsx" element={
          ...
          <Workbook.Column label="Type of Report" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    typeofreportService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Type of Report',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Type of Report',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Type Of Report',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Type of Report' : 'Create Type of Report'}   
          </ModalHeader>
          <ModalBody>
