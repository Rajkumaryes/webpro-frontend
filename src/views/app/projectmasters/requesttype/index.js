...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      requesttypeService.fetchrequesttype()
        .then((res) => { 
          this.setState({   
        loading : false 
...
      })
  ...
  onChangeFileUpload(files)
    ...
    requesttypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
                ...
                <div className = "row">
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Reason Code',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Request Type' : 'Create Request Type'}   
          </ModalHeader>
          <ModalBody>
