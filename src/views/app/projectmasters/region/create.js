...
createAPI(region_name,region_code,status)
{
  ...
  regionService.createregion(region_name,region_code,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,region_name,region_code,status)
{
  ...
  regionService.updateregion(id,region_name,region_code,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {region_name,region_code,isEdit,status,loading} = this.state
      ...
      <div>
        ...
        <Row>
            ...
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Region Code',languageData)} 
                        value = {region_code}  
                        onChange= {(e)=>this.setState({region_code : e.target.value})} ></Input>
                 </Colxx>
             </Row>
             <Row>
              <Colxx xxs="4" className="mb-4">
            ...
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Region Name',languageData)} 
                        value = {region_name}  
                        onChange= {(e)=>this.setState({region_name : e.target.value})} ></Input>
                 </Colxx>
