...
import{eposactivityService} from '../../../../redux/projectmasters/eposactivity/saga'
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    eposactivityService.deleteactivityepos(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Activity.xlsx" element={
          ...
          <Workbook.Column label="Activity" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    eposactivityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
              ...
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Activity (Error Handling Productivity)',languageData)} match={match} />
                    </div>
