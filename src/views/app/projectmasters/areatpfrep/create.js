...
import{areatpfrepService} from '../../../../redux/projectmasters/areatpfrep/saga'
...
onSubmit() { 
  const {id,area,region,status,isEdit} = this.state;

  if(area !== "")
  {
    if(isEdit === true)
    {
      this.updateAPI(id,area,region,status)
    }
    else
    {
      this.createAPI(area,region,status)
    }
...
createAPI(area,region,status)
{
  this.setState({
    loading : true
  })
  areatpfrepService.createarearegion(area,region,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,area,region,status)
{
  this.setState({
    loading : true
  })
  areatpfrepService.updatearearegion   (id,area,region,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {area,region,isEdit,status,loading} = this.state
          ...
          <div>
            ...
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        ...
                        value = {area}  
                        onChange= {(e)=>this.setState({area : e.target.value})} ></Input>
                 </Colxx>
