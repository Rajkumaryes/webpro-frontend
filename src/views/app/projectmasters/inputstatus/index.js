...
import{inputstatusService} from '../../../../redux/projectmasters/inputstatus/saga';
class MasterPage extends Component {
  ...
  handleDelete = id => {
    inputstatusService.deleteinputstatus(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Data.xlsx" element={
          ...
          <Workbook.Column label="Inputstatus" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    inputstatusService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Input Status',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Input Status',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Input Status',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Input Status' : 'Create Input Status'}   
          </ModalHeader>
          <ModalBody>
