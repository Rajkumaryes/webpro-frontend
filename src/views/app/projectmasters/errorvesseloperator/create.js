...
import{areaerrorService} from '../../../../redux/projectmasters/areaerror/saga';
import{regionerrorService} from '../../../../redux/projectmasters/regionerror/saga'
class RegisterUser extends Component {
 ...
 fetchregionwise(region) {
  ...
  areaerrorService.fetchAreaRegionWise(region)
  .then((res) => {
  if(res.status)   { 
    let filterstatus = (res.data).filter(item => item.status === 1)
...
handlechangeregion = (selectedOptions) => {
  this.setState({region : selectedOptions.value,area_data:[]})
  this.fetchregionwise(selectedOptions.value)
} 
onSubmit() { 
  const {id,name,region,area,status,isEdit} = this.state;

  if(name !=='' && region !=='' &&  area !=='' )
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,region,area,status)
    }
    else
    {
      this.createAPI(name,region,area,status)
    }
...
createAPI(name,region,area,status)
{
  this.setState({
    loading : true
  })
  errorvesseloperatorsService.createerrorvesseloperators(name,region,area,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,region,area,status)
{
  this.setState({
    loading : true
  })
  errorvesseloperatorsService.updateerrorvesseloperators(id,name,region,area,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {name,region,area,region_data,area_data,isEdit,status,loading} = this.state
      ...
      <div>
         ...
         <Row>
            ...
            <Select  
             ...
             <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label> {onChangeLanguage(locale,'Vessel Operator',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Vessel Operator',languageData)} 
                        value = {name}  
