...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      errorvesseloperatorsService.fetcherrorvesseloperators()
        .then((res) => { 
          this.setState({   
        loading : false 
 ...
 fetcharea() {
  ...
  handleDelete = id => {
    errorvesseloperatorsService.deleteerrorvesseloperators(id)
    .then((res) => {
      if(res){
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Vessel Operators',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-9">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Vessel Operators',languageData)} match={match} />
                    </div>
                    <div className = "col-md-3">
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Vessel Operators',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Vessel Operators' : 'Create Vessel Operators'}   
          </ModalHeader>
          <ModalBody>
