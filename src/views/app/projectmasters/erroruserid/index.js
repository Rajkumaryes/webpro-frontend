...
import{erroruseridService} from '../../../../redux/projectmasters/erroruserid/saga'
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      erroruseridService.fetcherroruserid()
        .then((res) => { 
          this.setState({   
        loading : false 
  ...
  handleDelete = id => {
    erroruseridService.deleteerroruserid(id)
    .then((res) => {
      if(res){
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Error UserId',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-10">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Error UserId',languageData)} match={match} />
                    </div>
                    <div className = "col-md-2">
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Error UserId',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Error UserId' : 'Create Error UserId'}   
          </ModalHeader>
          <ModalBody>
