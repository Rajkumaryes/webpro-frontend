...
import{activitytpfrepService} from '../../../../redux/projectmasters/activitytpfrep/saga'
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    activitytpfrepService.deleteactivitytpfrep(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Activity.xlsx" element={
          ...
          <Workbook.Column label="Activty" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    activitytpfrepService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
              ...
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Activity (TPFREP Productivity)',languageData)} match={match} />
                    </div>
