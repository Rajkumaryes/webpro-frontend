...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    eposareaService.deleteareaepos(id)
    .then((res) => {
      if(res){
...
      })
  ...
  onChangeFileUpload(files)
    ...
    eposareaService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
              ...
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-10">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Area (Error Handling Productivity)',languageData)} match={match} />
                    </div>
