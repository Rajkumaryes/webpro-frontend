...
import{typebulkService} from '../../../../redux/projectmasters/typebulk/saga'
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    typebulkService.deletetypebulk(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Activity Type.xlsx" element={
          ...
          <Workbook.Column label="Activity Type" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    typebulkService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Activity Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Activity Type (Adhoc / Bulk Upload Productivity)',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Activity Type',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Activity Type' : 'Create Activity Type'}   
          </ModalHeader>
          <ModalBody>
