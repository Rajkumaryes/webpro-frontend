...
class RegisterUser extends Component {
 ...
 fetchregionwise(region) {
  ...
  teamsService.regionwisefilter(region)
  .then((res) => {
  if(res.status)   { 
    let filterstatus = (res.data).filter(item => item.status === 1)
...
handlechangeregion = (selectedOptions) => {
  ...
  teamsService.regionwisefilter(selectedOptions.value.toString())
  .then((res) => {
  if(res.status)   { 
    let filterstatus = (res.data).filter(item => item.status === 1)
       var arealist = filterstatus.map(function(cusmaidid) {
...
createAPI(team_name,region_id,area_id,country_code,status)
{
  ...
  teamsService.createteams(team_name,region_id,area_id,country_code,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,team_name,region_id,area_id,country_code,status)
{
  ...
  teamsService.updateteams(id,team_name,region_id,area_id,country_code,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {team_name,region_id,area_id,country_code,region_data,area_data,isEdit,status,loading} = this.state
         ...
         <Row>
            ...
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        ...
                        value = {team_name}  
                        onChange= {(e)=>this.setState({team_name : e.target.value})} ></Input>
                 </Colxx>
             </Row>
             <Row>
              <Colxx xxs="4" className="mb-4">
