...
import{hlclformatService} from '../../../../redux/projectmasters/hlclformat/saga'
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    hlclformatService.deletehlclformat(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="hlclformat.xlsx" element={
          ...
          <Workbook.Column label="hlclformat" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    hlclformatService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Hlcl Format',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Hlcl Format',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Hlcl Format',languageData)}
                      </Button>
                    </div>
                  </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Hlcl Format' : 'Create Hlcl Format'}   
          </ModalHeader>
          <ModalBody>
