...
import{stturncodeService} from '../../../../redux/projectmasters/stturncode/saga'
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    stturncodeService.deletestturncode(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="StturnCode.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    stturncodeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Stturn Code',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Stturn Code',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Stturn Code',languageData)}
                      </Button>
                    </div>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Stturn Code' : 'Create Stturn Code'}   
          </ModalHeader>
          <ModalBody>
