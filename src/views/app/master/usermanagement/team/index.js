         ...
         if(res.status)   { 
            let filterstatus = (res.data).filter(item => item.status === 1)
               var regionlist = filterstatus.map(function(cusmaidid) {
                    return  {text : cusmaidid.region ,value : cusmaidid.id.toString()};
                 });  
                  this.setState({
...
          return {
            ...
            'Area' :record.area !== null ? getValue(area_data,'value','text',record.area):'' ,
            'Team' : record.name,
            'Geo Code' : record.country_code,
        };
      })
      return(
        ...
        <Workbook filename="Team.xlsx" element={
          ...
          <Workbook.Column label="Geo Code" value="Geo Code"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    teamsService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
              ...
              {
                title:  onChangeLanguage(locale,'Geo Code',languageData),
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Team',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Team',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Team',languageData)}
                      </Button>
                    </div>
