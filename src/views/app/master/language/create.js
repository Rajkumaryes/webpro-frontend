...
onSubmit() { 
  const {id,name,key,status,isEdit} = this.state;

  if(name !== "")
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,key,status)
    }
    else
    {
      this.createAPI(name,key,status)
    }
...
createAPI(name,key,status)
{
  this.setState({
    loading : true
  })
  languageService.createlanguage(name,key,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,key,status)
{
  this.setState({
    loading : true
  })
  languageService.updatelanguage(id,name,key,status)
    .then((res) => { 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {name,key,isEdit,loading,status} = this.state
      ...
      <div>
         ...
         <Row>
              <Colxx xxs="4" className="mb-4">
                   ...
                   <Label> {onChangeLanguage(locale,'Language Name',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
               
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Language Name',languageData)}
                        value = {name}  
             ...
             <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label>{onChangeLanguage(locale,'Language key',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
