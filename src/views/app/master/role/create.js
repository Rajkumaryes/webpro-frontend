  ...
  componentDidMount() {
    ...
    if(isEdit == true)
    {
      this.setState({
        ...
        color:record.color !== null ?record.color:this.state.color,
        isEdit:true
      })
...
onSubmit() { 
  const {id,name,status,color,isEdit} = this.state;

  if(name !== "")
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,status,color)
    }
    else
    {
      this.createAPI(name,status,color)
    }
...
createAPI(name,status,color)
{
  this.setState({
    loading : true
  })
  roleService.createrole(name,status,color)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,status,color)
{
  this.setState({
    loading : true
  })
  roleService.updaterole(id,name,status,color)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {name,isEdit,status,loading,color} = this.state
        ...
        {loading && 
         ...
         <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label>{onChangeLanguage(locale,'Role Name',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
               
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Role Name',languageData)}
                        value = {name}  
             ...
             <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label>{onChangeLanguage(locale,'Color',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
