   ...
   fetchexception() {
    ...
    fetchTeamData() {  
     ...
     fetchUserData(team_array) {
      ...
      console.log("team_array", team_array);
          ...
          if ((Date.parse(enddate) > Date.parse(startdate)))
           {
            ...
            const {username} = this.props
            const report_url =  'cloudbooking',
            menu= 'Booking Process' , 
            submenu='Cloud Booking',
            type = 'Raw Data'
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          bookingstatusdata,exception_partydata,cargodata} = this.state
        var columns= [
          {
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Cloud Booking',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <Breadcrumb heading={onChangeLanguage(locale,'Cloud Booking',languageData)} match={match} />
