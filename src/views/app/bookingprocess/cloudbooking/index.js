...
import{cloudbookingService} from '../../../../redux/bookingprocess/cloudbooking/saga'
...
class QueryResolveSheet extends Component {
    ...
    fetchCloudBookingCount(){
          ...
          cloudbookingService.fetchcloudbookingcount(username)
          .then((res) => {
             if(res.status)   { 
                    let filterstatus = res.data;
                    let lastdata = res.lastcount;
                    this.setState({ 
                    cloudbookingcount:filterstatus, 
                    cloudbookingcountlast:lastdata     
                    }) 
                   }
                   else{
   ...
   setCollapse() 
   {
     ...
     onSubmit() { 
      this.setState({ isSubmitting: true });
        const { id,exception_raised_date_time,date,booking_number,cu_match_code ,mr_matchcode ,
          booking_type ,reasons ,cargo_type ,final_status ,comments,  cargodata,region,area,subarea,csb_office,
          assigned_user_id,export_haulage,first_pol,received_date,function_, exception_party,exception_partydata,exception_resolved,
           exception_reason,crm_case_number, last_pod,end_pod,tariff_id,updated_start_time} = this.state;
        if( booking_number !==''&& cu_match_code!=='' && region !=='' &&area !=='' &&subarea!=='' && csb_office !=='' && 
         export_haulage !=='' && received_date !==''  && cargo_type !=='' && final_status!=='' && exception_resolved !=='' )
        {
            ...
            start_date=convertLocalToUTCDate(updated_start_time),
              ...
              cloudbookingService.createcloudbooking(id, username,exception_raised_date_time,start_date,end_date,region,area,subarea,booking_number,csb_office,assigned_user_id,
                cu_match_code ,mr_matchcode ,export_haulage ,receiveddate ,cargo_type,tariff_id,exception_party,exception_resolved,crm_case_number,
...
onChangeRadioExceptionResolved(value) {
   
    this.setState({ exception_resolved: value })
    // alert(this.state.exception_resolved)
}
onChangetime(date)
{
...
fetchData() {
  ...
  if(booking_number !== "" || function_ !=='')
  {
      ...
      cloudbookingService.search(booking_number,function_)
      .then((res) => {
...
onChangeFileUpload(files)
  {
    ...
    cloudbookingService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  getteamvalue(value) {
    ...
    if(mr_matchcode == ""){
      ...
      clipboard.readText().then((text)=>{
        ...
        var record = getValue_B2000(text)
       
        if (record.routing) {
            let routingArray = record.routing.split(" → "); // Split the routing string
          
            let first_pol = routingArray[0];
            let last_pod = routingArray[routingArray.length - 1]; 
            this.setState({
              first_pol: first_pol,
              last_pod: last_pod
            });
    ...
    let array = record.received_date.split(" ");

    let receivedDate = array[0]; // e.g., "04/25/2025"
    let receivedTime = array[1]; // e.g., "14:30" or "2:30"
    let meridian = array[2] || ""; // AM or PM (optional)
    
    let fullReceivedDateTime = `${receivedDate} ${receivedTime} ${meridian}`.trim();
    
    const format = meridian ? "MM/DD/YYYY h:mm A" : "MM/DD/YYYY HH:mm";
    
    // Convert to UTC moment
    const utcTime = moment.utc(fullReceivedDateTime, format);
    
    // Get region from user or record (for example: "Asia/Kolkata" or "Europe/Istanbul")
    const regionTimeZone = record.region === "Turkey" ? "Europe/Istanbul" : "Asia/Kolkata";
    
    // Convert to JS Date object
    const utcDate = utcTime.toDate();
    
    // Format to region-local time string using native Intl
    ...
    const localDateTimeStr = formatter.format(utcDate); // e.g., "04/25/2025, 20:00:00"
    
    // Convert formatted string back to moment object in that region
    const localTime = moment(localDateTimeStr, "MM/DD/YYYY, HH:mm:ss");
    
    // Add 45 minutes
    const addedTime = localTime.clone().add(15, "minutes");
    
    // Get current time in that region
    const currentDateStr = formatter.format(new Date());
    const currentTime = moment(currentDateStr, "MM/DD/YYYY, HH:mm:ss");
    
    // Compare
    var diffInSeconds = addedTime.diff(currentTime, "seconds");
    
    // console.log("UTC Time              :", utcTime.format("YYYY-MM-DD HH:mm:ss"));
    // console.log("Local Time (" + regionTimeZone + "):", localTime.format("YYYY-MM-DD HH:mm:ss"));
    // console.log("Time after 45 minutes :", addedTime.format("YYYY-MM-DD HH:mm:ss"));
    // console.log("Current Local Time    :", currentTime.format("YYYY-MM-DD HH:mm:ss"));
    // console.log("Time Difference (sec) :", diffInSeconds);
    ...
    this.setState(
      {
        ...
        export_haulage: record.export_haulage,
        first_pol: this.state.first_pol,
        last_pod: this.state.last_pod,
        end_pod: record.end_pod,
        issuer: record.issuer_office,
        received_date: this.validDate(fullReceivedDateTime, "Received Date"),
        temp: "",
        dg: "",
        oog: "",
      ...
      clipboard.readText().then((text)=>{
        //   var record = getValue_S9610(text)
          ...
          if (record.routing) {
            let routingArray = record.routing.split(" → ");
...
formatTime = (totalSeconds) => {
  ...
  return { minutes, seconds, isNegative: totalSeconds < 0 };
    ...
    render()
    {
        ...
        const { gsc_userid,date,start_time,end_time,booking_number,cu_match_code ,mr_matchcode ,
            booking_type ,reasons ,cargo_type ,final_status ,comments,  cargodata,
            bookingstatusdata, reasonsdata,handlingdata,bookingdata,teamdata,region,area,subarea,csb_office,exception_resolved,
            assigned_user_id,export_haulage,received_date,function_, exception_party,exception_partydata,bookingdiffminutes,bookingdiffseconds,
             exception_reason,exception_reasondata,crm_case_number, exception_raised_date_time,tariff_id, cloudbookingcount,cloudbookingcountlast,isDataPasted,
              ...
              },
              {
                ...
                },
                {
                  ...
                  },
                  {
                    ...
                    render: (text, record) => ( 
                      <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'200px'}}>
                        {getValue(cargodata, 'value', 'label', text)} 
                      </div>),
                  },
                  {
            ...
            <Row>
              <Colxx xxs="12"> 
                <div className = "row">
                      ...
                      <Breadcrumb heading="Cloud Booking" match={match} />
                    </div>
                    <div className = "col-md-2">
                        <h2 style = {{marginTop:'15px'}}>Total EQ : {cloudbookingcount}</h2>
                    </div>
                    <div className = "col-md-2">
                        <h2  style = {{marginTop:'15px'}}>Last EQ : {cloudbookingcountlast}</h2>
                    </div>
                  </div>
          ...
          {bookingdiffseconds !== null ? (
            ...
            <span style={{ fontWeight: "bold", fontSize: "28px" }}>--</span> // Placeholder if value is empty
                ...
                <div className = "publishuser-card-component" style = {{padding:'10px',borderRadius:'10px',marginBottom:'30px'}}>
                    ...
                    <div className = "col-md-3 space-margin">
                      ...
                      <div className="col-md-3">
                            <Label className="fontstyle normal-font" >{onChangeLanguage(locale, 'Exception Solved', languageData)}   <a style={{ color: 'red' }}>*</a></Label>
                            <p className='fontstyle mandatory-label'>{is_submit === true && exception_resolved === '' && `${onChangeLanguage(locale, 'Mandatory Field', languageData)}`}</p>
                            <div style={{ margin: '10px 0px' }}>
                                <Row>
                                    <Colxx xxs="4">
                                        <CustomRadioButton checked="Yes" name= {onChangeLanguage(locale, 'Yes', languageData)} value={exception_resolved}
                                            onChangeRadio={this.onChangeRadioExceptionResolved.bind(this)} />

                                    </Colxx>
                                    <Colxx xxs="4">
                                        <CustomRadioButton checked="No" name={onChangeLanguage(locale, 'No', languageData)} value={exception_resolved}
                                            onChangeRadio={this.onChangeRadioExceptionResolved.bind(this)} />
                          ...
                          <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Cargo Type',languageData)}<a style = {{color :'red'}}>*</a></Label>
                            <Select  
                                ...
                                classNamePrefix="react-select"
                                name="form-field-name"
                                value={cargodata.filter(option =>option.value === cargo_type)}
                                options={cargodata}
                ...
                <Button className = "button-width" color="secondary"
                      onClick={()=>this.onPasteB2000()}>
                    {onChangeLanguage(locale,'Paste B2000',languageData)}
                  </Button>  
