...
class QueryResolveSheet extends Component {
          ...
          if ((Date.parse(enddate)))
           {
            ...
            type = 'Raw Data'
            reportService.fetchbookingcreation(report_url,page,per_page,enddate,user_data,is_report,
                  ...
                  if(is_report !== true)
                  {
                    if(res.data)
                    {
                        ...
                        newData.forEach((item,index) => {
                          ...
                          tableobj ["Total EQ Value"] = (parseFloat(item.eqvalue1) || 0) + (parseFloat(item.eqvalue2) || 0) + (parseFloat(item.eqvalue3) || 0) + (parseFloat(item.eqvalue4) || 0) +
  ...
  handleChange = (e) => {
    ...
    render()
    {
            ...
            },
            {
              ...
              },
              {
                ...
                render: (_, record) => {
                  ...
                  return <div className="row d-flex justify-content-center" style = {{padding:'2px',width:'100px',color: totalEQValue >= 48 ? "green" : "red"}}>
                    {totalEQValue >= 48 ? <div><AiOutlineArrowUp/>{totalEQValue}</div>:<div><AiOutlineArrowDown/>{totalEQValue}</div>}
                    </div>;
                },
              },
        ]
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Booking Creation Dashboard',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <Breadcrumb heading={onChangeLanguage(locale,'Booking Creation Dashboard',languageData)} match={match} />
                          ...
                          <div className = "row" style = {{padding:'10px'}}>
                                ...
                                {(columns.length > 0 && data.length > 0) && 
                                  ...
                                  style = {{color :'white',padding:'8px 16px',border:'none',borderRadius:'4px',backgroundColor:'#121C50',cursor:'pointer',fontSize:'14px'}}
                                      data={exceldata} filename={`BookingCreation${moment(enddate).format('DD/MM/YYYY')}.csv`}
