...
import{finalstatusService} from '../../../../../redux/bookingprocess/finalstatus/saga'
...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    finalstatusService.deleteapi(id)
    .then((res) => {
      if(res){
...
      })
  ...
  onChangeFileUpload(files)
    ...
    finalstatusService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Final Status',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Final Status',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Final Status',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Final Status',languageData) : onChangeLanguage(locale,'Create Final Status',languageData)}   
          </ModalHeader>
          <ModalBody>
