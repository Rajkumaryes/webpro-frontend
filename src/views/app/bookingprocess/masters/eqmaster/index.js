...
import{eqmasterService} from '../../../../../redux/bookingprocess/eqmaster/saga';
        ...
        .then((res) => {
          ...
          if (res.status) {
            ...
            var regionlist = filterstatus.map(function (cusmaidid) {
              return {
                label: cusmaidid.name ,value: cusmaidid.id.toString(),text:cusmaidid.name
              };
            });
            this.setState({
          ...
          if(res.status)
            {
              let filterstatus = (res.data).filter(item => item.status === 1)
              var regionlist = filterstatus.map(function (cusmaidid) {
                return { label: cusmaidid.region,area: cusmaidid.area, value: cusmaidid.id.toString(),text: cusmaidid.region };
              });
...
          return {
            'Menu' :record.menu,
            'Name' : record.menu === 'Pending Follow-up Sheet' ?getValue(exception_type_data, 'value', 'label', record.name): record.menu === 'Booking Amendment' ?getValue(amendment_medium_data, 'value', 'label', record.name):record.name ,
        ...
        const columns = [
             {
              ...
              },
              {
                ...
                render: (text, record) => (	
                  <div>
                    {/* {record.module !== 'Pending Follow-up Sheet' && text !== null && text} */}
                    {record.menu === 'Pending Follow-up Sheet' ? getValue(exception_type_data, 'value', 'label', text):record.menu === 'Booking Amendment' ? getValue(amendment_medium_data, 'value', 'label', text):text  } 
                  </div>
              ),
