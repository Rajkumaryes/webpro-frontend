...
import{af_statusexceptionService} from '../../../../../redux/bookingprocess/af_statusexception/saga'
...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      af_statusexceptionService.fetchapi()
        .then((res) => { 
          this.setState({   
        loading : false 
  ...
  handleDelete = id => {
    af_statusexceptionService.deleteapi(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Container Type.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    af_statusexceptionService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Af Status Exception',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Af Status Exception',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Af Status Exception',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Af Status Exception',languageData) : onChangeLanguage(locale,'Create Af Status Exception',languageData)}   
          </ModalHeader>
          <ModalBody>
