...
import{ErrorActivityService} from '../../../../../redux/FeedersSchedules/feedermaster/erroractivitytype/saga'
...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      ErrorActivityService.fetcherroractivity()
        .then((res) => { 
          this.setState({   
        loading : false 
  ...
  handleDelete = id => {
    ErrorActivityService.deleteerroractivity(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="activitytype.xlsx" element={
          ...
          <Workbook.Column label="Error Activity Name" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    ErrorActivityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Activity',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Activity',languageData)} match={match} />
                    </div>
