...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    ActivityselectionService.deleteactivityselection(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="ActivitySelection.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    ActivityselectionService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Activity Selection',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Activity Selection',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Activity Selection',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Activity Selection' : 'Create Activity Selection'}   
          </ModalHeader>
          <ModalBody>
