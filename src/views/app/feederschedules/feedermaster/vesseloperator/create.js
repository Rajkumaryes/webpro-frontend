...
class RegisterUser extends Component {
  ...
  fetcharea(area) {
    vesselService.filterregionwise(area)
    .then((res) => {
       if(res.status)   { 
          let filterstatus = (res.data).filter(item => item.status === 1)
             var arealist = filterstatus.map(function(cusmaidid) {
...
createAPI(name,region,area,status)
{
  ...
  vesselService.createvessel(name,region,area,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,region,area,status)
{
  ...
  vesselService.updatevessel(id,name,region,area,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
handlechangeregion = (selectedOptions) => {
  ...
  this.fetcharea(selectedOptions.value)
  // vesselService.filterregionwise(selectedOptions.value)
  // .then((res) => {
  //    if(res.status)   { 
  //       let filterstatus = (res.data).filter(item => item.status === 1)
  //          var arealist = filterstatus.map(function(cusmaidid) {
  ...
  render()
  {
    ...
    const {name,area,region,isEdit,regiondata,areadata,status,loading} = this.state
      ...
      <div>
        ...
        <Row>
            ...
            <Select  
             ...
             <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label> {onChangeLanguage(locale,'Vessel Operatory',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Vessel Operatory',languageData)} 
                        value = {name}  
