...
class MasterPage extends Component {
  ...
  renderTemplate()
  {
    ...
    const {data,regiondata,areadata} = this.state
    var array = data.map(record=> {
          return {
            'name' : record.name,
            'region':getValue(regiondata,'value','text',parseInt(record.region)),
            'area':getValue(areadata,'value','text',parseInt(record.area)),
        };
      })
      return(
        
        <Workbook filename="Vessel.xlsx" element={
          ...
          <Workbook.Column label="Vessel Operator" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    vesselService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
          {
            ...
            render: (text, record) => (	
              <div>
                 {text !== null &&getValue(areadata,'value','text',parseInt(text))}
              </div>
          ),
          filters:areadata,
          filterMultiple: false,
          onFilter: (value, record) => parseInt(record.area) === parseInt(value),
          },
          {
            ...
            render: (text, record) => (	
              <div>
                 {text !== null &&getValue(regiondata,'value','text',parseInt(text))}
              </div>
          ),
          filters:regiondata,
          filterMultiple: false,
          onFilter: (value, record) => parseInt(record.region) === parseInt(value),
          },
             {
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Vessel Operator',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Vessel Operator',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Vessel Operator',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Vessel Operator' : 'Create Vessel Operator'}   
          </ModalHeader>
          <ModalBody>
