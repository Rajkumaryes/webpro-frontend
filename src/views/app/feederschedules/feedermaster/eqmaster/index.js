...
import{eqmasterService} from '../../../../../redux/FeedersSchedules/eqmaster/saga';
...
class MasterPage extends Component {
  ...
  renderTemplate()
  {
    ...
    const {data,activitytypedata} = this.state
    var array = data.map(record=> {
          return {
            'Name' : record.name  !== null ? getValue(activitytypedata,'value','text',record.name):'',
            'EQ' : record.eq,
        };
      })
      return(
        
        <Workbook filename="EQmaster.xlsx" element={
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        const {data,region_data,activitytypedata} = this.state
        const columns = [
           
              {
                ...
                render: (text, record) => (	
                        <div>
                           {text !== null &&getValue(activitytypedata,'value','text',text)}
                        </div>
                    ),
                    filters: activitytypedata,
