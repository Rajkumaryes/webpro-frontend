...
class MasterPage extends Component {
  ...
  renderTemplate()
  {
    ...
    const {data,activityselectiondata} = this.state
    var array = data.map(record=> {
          return {
            'name' : record.name,
            'activityselection':getValue(activityselectiondata,'value','text',parseInt(record.activityselection))
        };
      })
      return(
        ...
        <Workbook filename="Sub Activity.xlsx" element={
          ...
          <Workbook.Column label="Activity Selection" value="activityselection"/>
          <Workbook.Column label="Activity Name" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    subactivityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
              {
                title:  onChangeLanguage(locale,'Activity Selection',languageData),
                dataIndex: 'activityselection',
                key: 'activityselection',
                render: (text, record) => (	
                  <div>
                     {text !== null &&getValue(activityselectiondata,'value','text',parseInt(text))}
                  </div>
              ),
              filters:activityselectiondata,
              filterMultiple: false,
              onFilter: (value, record) => parseInt(record.activityselection) === parseInt(value),
              },
             {
                ...
                <div className = "row">
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Activity',languageData)}
                      </Button>
                    </div>
