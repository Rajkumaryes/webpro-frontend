...
onSubmit() { 
  const {id,name,sensitivity,status,isEdit} = this.state;

  if(name !== "" & sensitivity !=='')
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,sensitivity,status)
    }
    else
    {
      this.createAPI(name,sensitivity,status)
    }
...
createAPI(name,sensitivity,status)
{
  this.setState({
    loading : true
  })
  ErrortypeService.createerrortype(name,sensitivity,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,sensitivity,status)
{
  this.setState({
    loading : true
  })
  ErrortypeService.updateerrortype(id,name,sensitivity,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {name,sensitivity,isEdit,status,loading} = this.state
            ...
            <Loading loading={true} background="rgba(239,100,50, 0.1)" loaderColor="#EF6432" />
             ...
             <Row>
              <Colxx xxs="4" className="mb-4">
                   <Label> {onChangeLanguage(locale,'Error Type',languageData)}<a style = {{color :'red'}}>*</a></Label>
              </Colxx>
               
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Error Type',languageData)} 
                        value = {name}  
