...
import {regionServicerr} from '../../../../../redux/region/saga'
  ...
  componentDidMount() {
    ...
    if(isEdit == true)
    {
      this.setState({
        ...
        end_time_am:record.end_time_am,
        start_time_am:record.start_time_am,
...
onSubmit() { 
  const {id,name,start_time,end_time,start_time_am,end_time_am,status,isEdit} = this.state;

  if(name !== "")
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,parseInt(start_time),parseInt(end_time),start_time_am,end_time_am,status)
    }
    else
    {
      this.createAPI(name,parseInt(start_time),parseInt(end_time),start_time_am,end_time_am,status)
    }
...
createAPI(name,start_time,end_time,start_time_am,end_time_am,status)
{
  this.setState({
    loading : true
  })
  regionService.createregion(name,start_time,end_time,start_time_am,end_time_am,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,start_time,end_time,start_time_am,end_time_am,status)
{
  this.setState({
    loading : true
  })
  regionService.updateregion(id,name,start_time,end_time,start_time_am,end_time_am,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {name,region_data,start_time,end_time,starthour,endhour,start_time_am,end_time_am,starthourdata,endhourdata,isEdit,status,loading} = this.state
