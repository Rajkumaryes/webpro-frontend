...
import{reportService} from '../../../../redux/MIS/nonproductivityhoursrawdata/saga'
...
class QueryResolveSheet extends Component {
  ...
  if(team_id !== '')
  {
    var starthhdate=convertLocalToUTCDate(new Date())
    this.setState({
      startdate:new Date(finalDate),
      enddate:starthhdate
    })
    this.fetchPagination(1,25, new Date(finalDate),starthhdate,false)
  }
  else
  {
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          fordata,supervisordata,categorydata,} = this.state
        var columns= [
          {
          ...
          },
          {
            ...
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {getValue(fordata,'value','label',text)}  
              </div>),           
          },
          {
            ...
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
              {getValue(categorydata,'value','label',text)}     
              </div>),           
          },
          {
            title: onChangeLanguage(locale,'Supervisor Approval',languageData),
            dataIndex: 'supervisor',
            key: 'supervisor',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {getValue(supervisordata,'value','label',text)}    
              </div>),           
          },
          {
            title: onChangeLanguage(locale,'Non-Productivity Task Description',languageData),
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Non-Productive Hours Raw Data',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <Breadcrumb heading={onChangeLanguage(locale,'Non-Productive Hours Raw Data',languageData)} match={match} />
