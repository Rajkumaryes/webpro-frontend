...
import{reportService} from '../../../../redux/MIS/errorcapturerawdata/saga'
    ...
    fetchPagination(page,per_page,startdate,enddate,is_report)
    {
        ...
        if(enddate !== "" && startdate !== ""  && team_id !== "")
        {
          if ((Date.parse(enddate) > Date.parse(startdate)))
          {
            ...
            const menu= 'MIS QSC' , 
            submenu='Error Capture',
            type = 'Raw Data',
            audit_type= 'export'
            var date1 = new Date(startdate.setHours(startdate.getHours()+2))
            var date2 = moment(new Date(enddate)).format('YYYY-MM-DD HH:mm:ss a')
            reportService.fetchapi(page,per_page,date1,enddate,user_data,is_report,
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          checklist_data,area_data,teamdata} = this.state
        var columns= [
          {
          ...
          {
            title: onChangeLanguage(locale,'Error Reported By',languageData),
          ...
          {
            title: onChangeLanguage(locale,'Details',languageData),
            dataIndex: 'details',
            key: 'details',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
          ...
          {
            title: onChangeLanguage(locale,'Report',languageData),
            dataIndex: 'report',
            key: 'report',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Error Capture Raw Data',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <Breadcrumb heading={onChangeLanguage(locale,'Error Capture Raw Data',languageData)} match={match} />
