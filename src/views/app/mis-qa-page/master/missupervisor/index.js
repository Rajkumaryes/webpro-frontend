...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    missupervisorService.deletemissupervisor(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Supervisor Approval.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    missupervisorService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Supervisor Approval',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Supervisor Approval',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Supervisor',languageData)}
                      </Button>
                    </div>
