...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    misqscmasterService.deletemisqscmaster(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="MISQSC Task.xlsx" element={
          ...
          <Workbook.Column label="Report Name" value="name"/>
  ...
  onChangeFileUpload(files)
    ...
    misqscmasterService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Report Name',languageData),
              ...
              },
              {
                ...
                dataIndex: 'timetaken',
                key: 'timetaken',
                ...this.getColumnSearchProps('timetaken')
              },
              {
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'MISQSC Task',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'MISQSC Task',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add MISQSC Task',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update MISQSC Task' : 'Create MISQSC Task'}   
          </ModalHeader>
          <ModalBody>
