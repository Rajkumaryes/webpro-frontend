...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    misreportforService.deletemisreportfor(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Report For.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    misreportforService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Report For',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Report For',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Report For',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Report For' : 'Create Report For'}   
          </ModalHeader>
          <ModalBody>
