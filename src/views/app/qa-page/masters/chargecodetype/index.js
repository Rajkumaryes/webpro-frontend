...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    chargecodetypeService.deleteapi(id)
    .then((res) => {
      if(res){
...
      })
  ...
  onChangeFileUpload(files)
    ...
    chargecodetypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Type of Charge Code',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-6">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Type of Charge Code',languageData)} match={match} />
                    </div>
                    <div className = "col-md-6">
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Type of Charge Code',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Type of Charge Code',languageData) : onChangeLanguage(locale,'Create Type of Charge Code',languageData)}   
          </ModalHeader>
          <ModalBody>
