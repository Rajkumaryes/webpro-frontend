...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    otherscategoryService.deleteapi(id)
    .then((res) => {
      if(res){
...
      })
  ...
  onChangeFileUpload(files)
    ...
    otherscategoryService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Other Error Category',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Other Error Category',languageData)} match={match} />
                    </div>
