...
class InputAudit extends Component {
    ...
    onSubmit()
    {
        const {shipment_no,mtd_no,customer,correcter_id,amd_type,req_type,maf_added,team,
            start_datetime,region,error,correction_remarks,auditor_id,shipper,
             auditor_remarks,tl_acceptance,tl_remarks,received_datetime,start_time,updated_start_time} = this.state
        var end_datetime = moment(new Date()).format('MM/DD/YYYY hh:mm:ss a')
        if(error !== "" &&   auditor_id !== "" && auditor_remarks !== "" && shipper !=='')
        {
            ...
            otherauditstwoService.createapi(username,shipment_no,mtd_no,customer,correcter_id,amd_type,
    ...
    onPaste()
    {
        clipboard.readText().then((text)=>{
            var record = getValue_OtherAudit2(text)
    ...
    render()
    {
        ...
        const {shipment_no,mtd_no,customer,correcter_id,amd_type,req_type,maf_added,team,shipper,shipper_data,
            ...
            <>
                ...
                <div className = "publishuser-card-component" style = {{borderRadius:'10px',marginBottom:'30px'}}>
                   ...
                   <div className = "col-md-3 space-margin">
                       ...
                       <div className = "col-md-3 space-margin">
                           <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Corrector ID',languageData)}</Label>
                           <Input   className = {"fontstyle text-background-paste" } 
                           placeholder = ''
                           value = {correcter_id}  
                       ...
                       <div className = "col-md-3 space-margin">
                           <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Charges (MAF / SMC)',languageData)}</Label>
                           <Input   className = {"fontstyle text-background-paste" } 
                           placeholder = ''
                           value = {maf_added}  
                           onChange= {(e)=>this.setState({maf_added : e.target.value})} 
                           />
                       </div>
                       <div className = "col-md-3 space-margin">
                           <Label  className = "fontstyle normal-font" > {onChangeLanguage(locale,'Corrector Remarks',languageData)}</Label>
                            <Input   className = {"fontstyle text-background-paste" } 
                            placeholder = ''
                       ...
                       <div className = "col-md-3 space-margin">
                           <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'DC / SR Received Time',languageData)}</Label>
                           <Input   className = {"fontstyle text-background-paste" } 
                           placeholder = ''
