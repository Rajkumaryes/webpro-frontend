...
class InputAudit extends Component {
    ...
    onSubmit()
    {
        const {exception_issuedate,mtd_no,exception_no,subregion,errorcode,errorcode_group,receiver,area,
            start_datetime,region,error,user_of_lastaction,auditor_id,correct_error_code,issue_description,
        ...
        if(exception_issuedate !== "" &&  mtd_no !== "" &&  exception_no !== "" && errorcode !== "" &&  
           receiver !== "" &&   capture_user !== "" &&correct_error_code !== "" &&
            ...
            solvedby_capture_user !== "" && performed_action !== "")
        {
            ...
            errorcodeauditService.createapi(username,exception_issuedate,mtd_no,exception_no,subregion,errorcode,errorcode_group,receiver,area,
                start_datetime,end_datetime,region,error,user_of_lastaction,auditor_id,correct_error_code,issue_description,
                 auditor_remarks,tl_acceptance,tl_remarks,capture_user,solvedby_capture_user,exception_status,
    ...
    onPaste()
    {
        clipboard.readText().then((text)=>{
            var record = getValue_ErrorCode(text)
            ...
            this.setState({
                ...
                correct_error_code:record.correct_error_code,
    ...
    render()
    {
        ...
        const {exception_issuedate,mtd_no,exception_no,subregion,errorcode,errorcode_group,receiver,area,correct_error_code,correct_error_code_data,
                ...
                <div className = "publishuser-card-component" style = {{borderRadius:'10px',marginBottom:'30px'}}>
                   ...
                   <div className = "col-md-3 space-margin">
                       ...
                       <div className = "col-md-3 space-margin">
                           ...
                           <Input   className = {is_submit === true && errorcode === ''?  "error-border-paste":"fontstyle text-background-paste" }
                           placeholder = ''
                           value = {errorcode}  
                       ...
                       <div className = "col-md-3 space-margin">
                           <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Date of Creation (MTD)',languageData)}<a style = {{color :'red'}}>*</a></Label>
                           <Input   className = {is_submit === true && dod === ''?  "error-border-paste":"fontstyle text-background-paste" } 
                           placeholder = ''
                           value = {dod}  
                       ...
                       <div className = "col-md-3 space-margin">
                           <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'Correct Error code',languageData)}<a style = {{color :'red'}}>*</a></Label>
                           {/* <Select className={is_submit === true && correct_error_code === ''?  "error-border-select":"react-select fontstyle" }
                            classNamePrefix="react-select"
                            name="form-field-name"
                            value={correct_error_code_data.filter(option =>option.value === correct_error_code)}
                            options={correct_error_code_data}
                            onChange={(option) => this.setState({  correct_error_code: option.value })}
                            /> */}
                             <Input   className = {is_submit === true && correct_error_code === ''?  "error-border":"fontstyle text-background" } 
                           placeholder = ''
                           value = {correct_error_code}  
                           onChange= {(e)=>this.setState({correct_error_code : e.target.value})} 
                           />
                       </div>
                       <div className = "col-md-3 space-margin">
