...
class MasterPage extends Component {
  ...
  onChangeFileUpload(files)
    ...
    invoiceService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="invoice.xlsx" element={<Button  className = "button-width" color="secondary">
      {onChangeLanguage(locale,'Download',languageData)}
      </Button>}>
      <Workbook.Sheet data={array} name="Sheet A">
      <Workbook.Column label="Name" value="Name"/>
      <Workbook.Column label="Invoice Type" value="Invoice Type"/>
      </Workbook.Sheet>
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
              ...
              {
                title:  onChangeLanguage(locale,'Invoice Type',languageData),
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Invoice Status',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Invoice Status',languageData)} match={match} />
                    </div>
                    <div className = "col-md-4">
                        ...
                        {onChangeLanguage(locale,' Add Invoice Status',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Invoice Status',languageData) : onChangeLanguage(locale,'Create Invoice Status',languageData)}   
          </ModalHeader>
          <ModalBody>
