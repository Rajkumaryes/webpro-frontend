...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    auditstatusService.deleteapi(id)
    .then((res) => {
      if(res){
  ...
  onChangeFileUpload(files)
    ...
    auditstatusService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="Audit_Status.xlsx" element={<Button  className = "button-width" color="secondary">
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Audit Status',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Audit Status',languageData)} match={match} />
                    </div>
                    <div className = "col-md-4">
                        ...
                        {onChangeLanguage(locale,' Add Audit Status',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Audit Status',languageData) : onChangeLanguage(locale,'Create Audit Status',languageData)}   
          </ModalHeader>
          <ModalBody>
