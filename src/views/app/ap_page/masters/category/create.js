...
onSubmit() { 
  const {id,name,status,isEdit,sensitivity} = this.state;

  if(name !== "" && sensitivity !== '')
  {
    if(isEdit === true)
    {
      this.updateAPI(id,name,status,sensitivity)
    }
    else
    {
      this.createAPI(name,status,sensitivity)
    }
...
createAPI(name,status,sensitivity)
{
  this.setState({
    loading : true
  })
  categoryService.createapi(name,status,sensitivity)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,name,status,sensitivity)
{
  this.setState({
    loading : true
  })
  categoryService.updateapi(id,name,status,sensitivity)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {name,isEdit,status,loading,sensitivity} = this.state
