...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    standardcommentService.deleteapi(id)
    .then((res) => {
      if(res){
  ...
  onChangeFileUpload(files)
    ...
    standardcommentService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        { this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
      ...
            })
    return(
      <Workbook filename="Standard Comments.xlsx" element={<Button  className = "button-width" color="secondary">
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Standard Comments',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Standard Comments',languageData)} match={match} />
                    </div>
                    <div className = "col-md-4">
                        ...
                        {onChangeLanguage(locale,' Add Standard Comments',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Standard Comments',languageData) : onChangeLanguage(locale,'Create Standard Comments',languageData)}   
          </ModalHeader>
          <ModalBody>
