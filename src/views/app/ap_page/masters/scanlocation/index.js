...
class MasterPage extends Component {
 ...
 fetcharea() {
  ...
  handleDelete = id => {
    scanlocationService.deleteapi(id)
    .then((res) => {
      if(res){
  ...
  onChangeFileUpload(files)
    ...
    scanlocationService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {this.setState({
          loading : false
  ...
  renderTemplate()
  {
    ...
    var array = data.map(record=> {
                return {
                  'Region' : getValue(region_data,'value','text',record.region),
                  'Area' : getValue(area_data,'value','text',record.area),
                  'Name' : record.name,
                  'EQ' : record.eq,
              };
            })
    return(
      <Workbook filename="Scan Location.xlsx" element={<Button  className = "button-width" color="secondary">
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Scan Location',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Scan Location',languageData)} match={match} />
                    </div>
                    <div className = "col-md-4">
                        ...
                        {onChangeLanguage(locale,' Add Scan Location',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Scan Location',languageData) : onChangeLanguage(locale,'Create Scan Location',languageData)}   
          </ModalHeader>
          <ModalBody>
