...
import { Label,Input,Button,CustomInput} from 'reactstrap';
...
onSubmit() { 
  const {id,region,name,status,isEdit} = this.state;

  if(region !== "" && name !=='')
  {
    if(isEdit === true)
    {
      this.updateAPI(id,region,name,status)
    }
    else
    {
      this.createAPI(region,name,status)
    }
...
createAPI(region,name,status)
{
  this.setState({
    loading : true
  })
  areasService.createapi(region,name,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,region,name,status)
{
  this.setState({
    loading : true
  })
  areasService.updateapi(id,region,name,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {region,name,region_data,isEdit,status,team_data,loading} = this.state
        ...
        <Row>
            ...
            <Colxx xxs="8" className="mb-4">
                   <Select  
                        ...
                        classNamePrefix="react-select"
                        name="form-field-name"
                        value={team_data.filter(option =>(option.value).toLowerCase() === name.toLowerCase())}
                        options={team_data}
