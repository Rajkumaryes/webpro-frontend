...
import {onChangeLanguage,get_array_id,convertLocalToUTCDate,getAreaValue,getoptionvalue} from '../../../../helper'
...
class QueryResolveSheet extends Component {
    ...
    fetchInvoiceCount(){
      ...
      invoiceallocationService.fetchinvoicecount(username)
      .then((res) => {
         if(res.status)   { 
                let filterstatus = res.data;
                let lastdata = res.lastcount;
                this.setState({ 
                invoicecountcount:filterstatus, 
                invoicecountcountlast:lastdata     
                }) 
               }
               else{
  ...
  fetchuser() {  
    ...
    getData()
    {
      this.setState({submit_column:true,submit_clear:true,open_popup:''})
    }
    clearData()
    {
    ...
    onSubmitClear()
    {
      const { selectedRowKeys,page,pageSize,region_filter,area_filter,username,companycode_filter, 
        vendor_filter,reference_filter,scanlocation_filter,document_id_filter,user_status,tabvalue} = this.state
              ...
              if(res.status)
                {
                 ...
                 this.fetchPagination(page,pageSize,region_filter,area_filter,document_id_filter,companycode_filter,
                  username,reference_filter,scanlocation_filter,vendor_filter,user_status,tabvalue)
    ...
    onSubmitClearAll()
    {
      ...
      if(all_data.length > 0)
      {
          ...
          invoiceallocationService.clearAll()
            .then((res) => { 
              this.setState({   
                loading : false,
                open_clearall:''   
    ...
    { 
      ...
      const index = data.findIndex(x => x.id === id)
        if(index >= 0)
        {
          ...
          this.onSubmitRecord(data[index],true)
        }
    }
    onSubmitData()
    {
      const { selectedRowKeys,all_data,user_status} = this.state
     

      if(all_data.length > 0 && selectedRowKeys.length > 0)
      {
         ...
         if(isfill === true)
         {
          ...
          invoiceallocationService.multiupdate_api(list)
            .then((res) => { 
              this.setState({   
                loading : false,
                open_movecomplete:''    
    ...
    onSubmitRecord(record,is_submit)
    {
    ...
    setTabvlaue(tab)
    {
      const {region_filter,area_filter,username,companycode_filter, vendor_filter,reference_filter,
        scanlocation_filter,document_id_filter,user_status} = this.state
      this.setState({
        tabvalue:tab,
        is_summary:false,
      })
     this.fetchPagination(1,25,region_filter,area_filter,document_id_filter,companycode_filter,
      username,reference_filter,scanlocation_filter,vendor_filter,user_status,tab)
    }
    setuserReport(user_status)
    {
      const {region_filter,area_filter,username,companycode_filter, vendor_filter,reference_filter,
        scanlocation_filter,document_id_filter,tabvalue} = this.state
      this.setState({
    ...
    setuserSummary()
    {
     ...
     this.fetchPagination(1,25,region_filter,area_filter,document_id_filter,companycode_filter,
      username,reference_filter,scanlocation_filter,vendor_filter,false,tabvalue)
    }
      ...
      username,reference_filter,scanlocation_filter,vendor_filter,user_status,tabvalue)
    {
      ...
      invoiceallocationService.filter_api(page,per_page,region,area,document_id,companycode,user_id,reference,scanlocation,vendor,invoice_status,user_status,tabvalue)
      .then((res) => { 
        this.setState({   
          loading : false     
        }) 
    ...
    render()
    {
       const {match,locale,languageData} = this.props
        const {username,all_data,area_data,selectedRowKeys,region_data,is_submit_table,area_filter,region_filter,invoicecount,invoicecountlast,
          tabvalue,submit_column,area_all_data,column_value,open_popup,open_clear,open_clearall,open_movecomplete,user_data,loading,user_status,
          companycode_filter, vendor_filter,reference_filter,scanlocation_filter,document_id_filter,
          is_summary,total_count,pending_count,complete_count} = this.state
          ...
          const  column_data = [
            ...
            },
            {
             ...
             {
              title:onChangeLanguage(locale,'Current Agent',languageData) ,
              dataIndex: 'current_agent',
              key: 'current_agent',
               render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {text}    
              </div>),
              sorter: (a, b) => (a.current_agent != null ?
                a.current_agent : "").localeCompare(b.current_agent !== null ? 
                b.current_agent : ""),
                sortDirections: ['descend', 'ascend'],
            },  
            {
            ...
            {
              title: onChangeLanguage(locale,'Current User',languageData),
              ...
              render: (text, record,index) => ( 
                ...
                sorter: (a, b) => (a.currentuser != null ?
                  a.currentuser : "").localeCompare(b.currentuser !== null ? 
                  b.currentuser : ""),
                  sortDirections: ['descend', 'ascend'],
            },
             {
             ...
             {
              title: onChangeLanguage(locale,'Start On',languageData),
              dataIndex: 'start_on',
              key: 'start_on',
              render: (text, record,index) => ( 
                <div  style = {{padding:'2px',width:'100px'}}>
             ...
             {
              title:onChangeLanguage(locale,'Start At',languageData) ,
              dataIndex: 'start_at',
              key: 'start_at',
              render: (text, record,index) => ( 
                <div  style = {{padding:'2px',width:'100px'}}>
            ...
            {
              title:onChangeLanguage(locale,'Document Archive User',languageData) ,
              dataIndex: 'archive_user',
              key: 'archive_user',
              render: (text, record,index) => ( 
                <div  style = {{padding:'2px',width:'100px'}}>
                 {text}    
...
                     })
        ...
        return (
            ...
            <title>{onChangeLanguage(locale,'Invoice Allocation',languageData)}</title>
            <Row>
              <Colxx xxs="12">
              <div className = "row">
                      <div className = "col-md-4">
                      <Breadcrumb heading={onChangeLanguage(locale,'Invoice Allocation',languageData)} match={match} />
                    </div>
      ...
      if(is_fill === true &&  no_of_task !== '' && region.length > 0 && area.length > 0 && user.length > 0)
      {
        if(parseInt(no_of_task) > 0)
        {
          var userarray = get_array_id(user),
              regions = get_array_id(region),
              areas = get_array_id(area) 
          const {username} = this.props
          if(is_reassign === true)
          {
              this.reassign_User(userarray,regions,areas,parseInt(no_of_task),username)
          }
          else
          {
            this.fileUpload(file,userarray,regions,areas,parseInt(no_of_task),username)
          }
    ...
    fileUpload(file,userarray,region,area,no_of_task,username)
    {
      this.closeModal()
      invoiceallocationService.fileUpload(file,userarray,region,area,no_of_task,username)
        .then((res) => { 
    ...
    reassign_User(userarray,region,area,no_of_task,username)
    {
      this.setState({
        loading : true
      })
      this.closeModal()
      invoiceallocationService.reassign_api(userarray,region,area,no_of_task,username)
        .then((res) => { 
          this.setState({   
            loading : false     
          }) 
            ...
            >
              ...
              <ModalBody>
                         <Row>
                            ...
                            <Row >
                                 <Colxx xxs="4"> <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'No. Of Task',languageData)}<a style = {{color :'red'}}>*</a></Label></Colxx>
                                <Colxx xxs="8">
