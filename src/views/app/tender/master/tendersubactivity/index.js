...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      tendersubactivityService.fetchtendersubactivity()
        .then((res) => { 
          this.setState({   
        loading : false 
  ...
  handleDelete = id => {
    tendersubactivityService.deletetendersubactivity(id)
    .then((res) => {
      if(res){
  ...
  renderTemplate()
  {
    ...
    const {data,activitydata} = this.state
    var array = data.map(record=> {
          return {
            'subactivity' : record.name,
            'activity' : getValue(activitydata,'value','text',parseInt(record.activity)),
        };
      })
      return(
        
        <Workbook filename="Sub Activity.xlsx" element={
          ...
          <Workbook.Column label="subactivity" value="subactivity"/>
  ...
  onChangeFileUpload(files)
    ...
    tendersubactivityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
              {
                ...
                render: (text, record) => (	
                  <div>
                     {text !== null &&getValue(activitydata,'value','text',parseInt(text))}
                  </div>
              ),
              filters:activitydata,
              filterMultiple: false,
              onFilter: (value, record) => parseInt(record.activity) === parseInt(value),
              },
              // {
              //   title:  onChangeLanguage(locale,'Sub Activity',languageData),
              //   dataIndex: 'name',
              //   key: 'name',
              // },
              {
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Sub Activity',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Sub Activity',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Sub Activity',languageData)}
                      </Button>
                    </div>
    ...
    renderModal()
  {
    const {modalOpen,record,isEdit,isadmin,activitydata} = this.state
    return (
        <Modal
          isOpen={modalOpen}
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Sub Activity' : 'Create Sub Activity'}   
          </ModalHeader>
          <ModalBody>
