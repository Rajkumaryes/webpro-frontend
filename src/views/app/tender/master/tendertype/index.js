...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    TendertypeService.deletetendertype(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Tender Type.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    TendertypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Tender Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Tender Type',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Tender Type',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Tender Type' : 'Create Tender Type'}   
          </ModalHeader>
          <ModalBody>
