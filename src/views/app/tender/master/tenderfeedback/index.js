...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    tenderfeedbackService.deletetenderfeedback(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Feedback.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    tenderfeedbackService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Feedback',languageData),
                dataIndex: 'name',
                key: 'name',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Feedback',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Feedback',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Feedback',languageData)}
                      </Button>
                    </div>
