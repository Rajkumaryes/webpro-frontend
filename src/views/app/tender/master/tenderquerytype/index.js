...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    tenderquerytypeService.deletetenderquerytype(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Query Type.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    tenderquerytypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Query Type' : 'Create Query Type'}   
          </ModalHeader>
          <ModalBody>
