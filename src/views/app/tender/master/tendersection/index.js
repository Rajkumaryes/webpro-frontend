...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    tendersectionService.deletetendersection(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Section.xlsx" element={
          ...
          <Workbook.Column label="Sub Section" value="subactivity"/>
  ...
  onChangeFileUpload(files)
    ...
    tendersectionService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Section',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Section',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Section',languageData)}
                      </Button>
                    </div>
