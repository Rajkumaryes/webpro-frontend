   ...
   fetchsection() {
    ...
    .then((res) => {
       if(res.status)   { 
          let filterstatus = (res.data).filter(item => item.status === 1)
             var regionlist = filterstatus.map(function(cusmaidid) {
                  return  {text : cusmaidid.name ,subactivity : cusmaidid.subactivity,value : cusmaidid.id.toString()};
               }); 
               var list = filterstatus.map(function(cusmaidid) {
                return  {text : cusmaidid.subactivity ,value : cusmaidid.id.toString()};
             });  
             var eallyUniqueArr =  list.filter((v,i,a)=>a.findIndex(t=>(t.text === v.text && t.value===v.value))===i)
                this.setState({
                subactivitydata :  regionlist,
...
          return {
            'Activity' :record.activity !== null ? getValue(activitydata,'value','text',record.activity):'' ,
            'Section' :record.section !== null ? getValue(subactivitydata,'value','text',record.section):'' ,
            'Sub Section' :record.subactivity,
            'EQ' : record.eq,
        };
      })
      return(
        ...
        <Workbook filename="EQ.xlsx" element={
          ...
          <Workbook.Column label="Section" value="Section"/>
          <Workbook.Column label="Sub Section" value="Sub Section"/>
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        const {data,activitydata,subactivitydata,subsectiondata} = this.state
        const columns = [
             {
                ...
                render: (text, record) => (	
                  <div>
                     {text !== null &&getValue(activitydata,'value','text',text)}
                  </div>
              ),
              ...
              },
              {
                ...
                render: (text, record) => (	
                  <div>
                     {text !== null &&getValue(subactivitydata,'value','text',text)}
                  </div>
              ),
              filters: subactivitydata,
              filterMultiple: false,
              ...
              },
              {
                ...
                key: 'subactivity',
                ...this.getColumnSearchProps('subactivity')
              },
