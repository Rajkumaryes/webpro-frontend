...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    tendererrorseverityService.deletetendererrorseverity(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Error Severity.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    tendererrorseverityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Error Severity',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Error Severity',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Error Severity',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Error Severity' : 'Create Error Severity'}   
          </ModalHeader>
          <ModalBody>
