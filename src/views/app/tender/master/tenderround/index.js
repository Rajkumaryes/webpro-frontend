...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    tenderroundService.deletetenderround(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Tender Round.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    tenderroundService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Tender Round',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Tender Round',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,' Add Tender Round',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Tender Round' : 'Create Tender Round'}   
          </ModalHeader>
          <ModalBody>
