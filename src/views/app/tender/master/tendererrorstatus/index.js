...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    tendererrorstatusService.deletetendererrorstatus(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Error Status.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    tendererrorstatusService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? 'Update Error Status' : 'Create Error Status'}   
          </ModalHeader>
          <ModalBody>
