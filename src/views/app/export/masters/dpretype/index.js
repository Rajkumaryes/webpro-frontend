...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    dpretypeService.deletedpretype(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="DPREtype.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    dpretypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  getColumnSearchProps = dataIndex => ({
		...
		filterIcon: filtered => <a style = {{color:'white',top:'10px',position:'absolute'}}><i className = "simple-icon-magnifier"/></a>,
  ...
  // };
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'DPRE Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'DPRE Type',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add DPRE Type',languageData)}
                      </Button>
                    </div>
                  </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update DPRE Type' , languageData) : onChangeLanguage(locale,'Create DPRE Type' , languageData) }   
          </ModalHeader>
          <ModalBody>
