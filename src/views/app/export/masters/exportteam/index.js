...
class MasterPage extends Component {
       ...
       if(res.status)   { 
          let filterstatus = (res.data).filter(item => item.status === 1)
             var regionlist = filterstatus.map(function(cusmaidid) {
                  return  {text : cusmaidid.region_code ,value : cusmaidid.id};
               });  
                this.setState({
 ...
 fetcharea() {
  ...
  renderTemplate()
  {
    ...
    const {data,region_data,area_data} = this.state
    var array = data.map(record=> {
          return {
           'region_id' : record.region_id !== null &&getValue(region_data,'value','text',parseInt(record.region_id)) ,
            'area_id' : record.area_id!== null &&getValue(area_data,'value','text',parseInt(record.area_id)) ,
...
      })
      return(
        ...
        <Workbook filename="Teams.xlsx" element={
          ...
          <Workbook.Column label="Area" value="area_id"/>
          <Workbook.Column label="Team" value="team_name"/>
          <Workbook.Column label="Country Code" value="country_code"/>
  ...
  onChangeFileUpload(files)
  {
    ...
    teamService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {  this.setState({
          loading : false
  ...
  getColumnSearchProps = dataIndex => ({
    ...
    filterIcon: filtered => <a style = {{color:'white',top:'5px',position:'absolute',    right: '12px',right:'12px'}}><i className = "simple-icon-magnifier"/></a>,
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             ...
             onFilter: (value, record) => parseInt(record.region_id) === value,
              },
              {
              ...
              filters: area_data,
              filterMultiple: false,
             onFilter: (value, record) => parseInt(record.area_id) === value,
              },
              {
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Teams' , languageData) : onChangeLanguage(locale,'Create Teams' , languageData) }      
          </ModalHeader>
          <ModalBody>
