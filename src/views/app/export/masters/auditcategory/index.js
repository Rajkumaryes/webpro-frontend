...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    auditcategoryservice.deleteauditcategory(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="AuditCategory.xlsx" element={
  ...
  onChangeFileUpload(files)
    ...
    auditcategoryservice.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Audit Category',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Audit Category',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Audit Category',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Audit Category' , languageData) : onChangeLanguage(locale,'Create Audit Category' , languageData) }   
           </ModalHeader>
          <ModalBody>
