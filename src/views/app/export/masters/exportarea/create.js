...
onSubmit() { 
  const {id,region_id,area_name,status,isEdit} = this.state;

  if(region_id !== "" && area_name !=='')
  {
    if(isEdit === true)
    {
      this.updateAPI(id,region_id,area_name,status)
    }
    else
    {
      this.createAPI(region_id,area_name,status)
    }
...
createAPI(region_id,area_name,status)
{
  this.setState({
    loading : true
  })
  areaexportservice.createarea(region_id,area_name,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
updateAPI(id,region_id,area_name,status)
{
  this.setState({
    loading : true
  })
  areaexportservice.updatearea(id,region_id,area_name,status)
    .then((res) => { 
      this.setState({   
        loading : false     
      }) 
...
close(isedit)
{
  ...
  render()
  {
    ...
    const {region_id,area_name,region_data,area_data,isEdit,status,loading} = this.state
      ...
      <div>
        ...
        <Row>
            ...
            <Select  
                        ...
                        classNamePrefix="react-select"
                        name="form-field-name"
                        value={area_data.filter(option =>(option.value).toLowerCase() === area_name.toLowerCase() )}
                        options={area_data}
                        onChange={({value}) => this.setState({  area_name: value })}
                      />
                 </Colxx>
             </Row>
{/* 
            ...
            <Colxx xxs="8" className="mb-4">
                  <Input 
                        type="text"
                        placeholder = {onChangeLanguage(locale,'Area Name',languageData)} 
                        value = {area_name}  
                        onChange= {(e)=>this.setState({area_name : e.target.value})} ></Input>
                 </Colxx>
