...
class MasterPage extends Component {
         ...
         if(res.status)   { 
            let filterstatus = (res.data).filter(item => item.status === 1)
               var regionlist = filterstatus.map(function(cusmaidid) {
                    return  {text : cusmaidid.region_name ,value : cusmaidid.id.toString()};
                 });  
                  this.setState({
  ...
  handleDelete = id => {
    areaexportservice.deletearea(id)
    .then((res) => {
      if(res){
...
          return {
             'region_id' : record.region_id!== null ? getValue(region_data,'value','text',record.region_id):'',
...
      })
      return(
        ...
        <Workbook filename="Area.xlsx" element={
          ...
          <Workbook.Column label="Region Name" value="region_id"/>
          <Workbook.Column label="Area Name" value="area_name"/>
  ...
  onChangeFileUpload(files)
    ...
    areaexportservice.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {  this.setState({
          loading : false
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Area' , languageData) : onChangeLanguage(locale,'Create Area' , languageData) }   
            </ModalHeader>
          <ModalBody>
