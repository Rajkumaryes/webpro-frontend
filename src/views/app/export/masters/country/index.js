...
class MasterPage extends Component {
         ...
         if(res.status)   { 
             let filterstatus = (res.data).filter(item => item.status === 1)
               var regionlist = filterstatus.map(function(cusmaidid) {
                    return  {text : cusmaidid.region_code ,value : cusmaidid.id.toString()};
                 });  
                  this.setState({
  ...
  handleDelete = id => {
    matchcountryService.deletematchcountry(id)
    .then((res) => {
      if(res){
...
          return {
            'region_id' : record.region_id!== null ? getValue(regiondata,'value','text',record.region_id):'',
            'name' : record.name,
...
      })
      return(
        ...
        <Workbook filename="matchcountry.xlsx" element={
          ...
          <Workbook.Column label="Region" value="region_id"/>
          <Workbook.Column label="Country" value="name"/>
          ...
          <Workbook.Column label="Email Adress" value="email_address"/>
  ...
  onChangeFileUpload(files)
    ...
    matchcountryService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {  this.setState({
          loading : false
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
          {
            ...
            render: (text, record) => (	
              <div>
                 {text !== null &&getValue(regiondata,'value','text',text)}
              </div>
          ),
          filters: regiondata,
          filterMultiple: false,
          onFilter: (value, record) => record.region_id === value,
          },
            
          {
              ...
              render: (text, record) => ( 
                <span className="row d-flex justify-content-center">                             
                   {(text === true)&& onChangeLanguage(locale,'TRUE',languageData)}
                </span>
                 ),  
              ...
              {
                title:  onChangeLanguage(locale,'Tax Code',languageData),
                dataIndex: 'taxcode',
                key: 'taxcode',
                render: (text, record) => ( 
                  <span className="row d-flex justify-content-center">                             
              ...
              {
                title:  onChangeLanguage(locale,'Email Address',languageData),
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Country',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Country',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Country',languageData)}
                      </Button>
                    </div>
                  </div>
    ...
    renderModal()
  {
    ...
    return (
        ...
        >
          ...
          {isEdit ? onChangeLanguage(locale,'Update Country' , languageData) : onChangeLanguage(locale,'Create Country' , languageData) }   
         </ModalHeader>
          <ModalBody>
