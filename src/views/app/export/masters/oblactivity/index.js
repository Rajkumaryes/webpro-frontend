...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    oblactivityService.deleteoblactivity(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="OBLActivity.xlsx" element={
          ...
          <Workbook.Column label="OBLActivity" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    oblactivityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
              ...
              {
                title:  onChangeLanguage(locale,'OBL/Customs Activity',languageData),
                dataIndex: 'name',
                key: 'name',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'OBL Activity',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'OBL Activity',languageData)} match={match} />
                    </div>
                    ...
                    {/* <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add OBL Activity',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update OBL Activity' , languageData) : onChangeLanguage(locale,'Create OBL Activity' , languageData) }    
          </ModalHeader>
          <ModalBody>
