...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    typeofotService.deletetypeofot(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Typeofot.xlsx" element={
          ...
          <Workbook.Column label="Typeofot" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    typeofotService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {  this.setState({
          loading : false
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Type of OT',languageData),
                dataIndex: 'name',
                key: 'name',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Type of OT',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Type of OT',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Type of OT',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
           {isEdit ? onChangeLanguage(locale,'Update Type of OT' , languageData) : onChangeLanguage(locale,'Create Type of OT' , languageData) }   
          </ModalHeader>
          <ModalBody>
