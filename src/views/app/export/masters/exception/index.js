...
import{exceptionService} from '../../../../../redux/Export/masters/exception/saga'
...
class MasterPage extends Component {
    ...
    fetchData() {  
      ...
      exceptionService.fetchexception()
        .then((res) => { 
          this.setState({   
        loading : false 
...
      })
      return(
        ...
        <Workbook filename="Exception.xlsx" element={
          ...
          <Workbook.Column label="Exception" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Exception',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Exception',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Exception',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Exception' , languageData) : onChangeLanguage(locale,'Create Exception' , languageData) }   
          </ModalHeader>
          <ModalBody>
