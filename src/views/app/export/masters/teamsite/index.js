...
class MasterPage extends Component {
         ...
         if(res.status)   { 
             let filterstatus = (res.data).filter(item => item.status === 1)
               var teams = filterstatus.map(function(response) {
                    return  {text : response.team_name ,value : response.id.toString()};
                 });  
                  this.setState({
                  team_data :  teams,
                  loading:false
  ...
  handleDelete = id => {
    teamsiteService.deleteteamsite(id)
    .then((res) => {
      if(res){
  ...
  renderTemplate()
  {
    ...
    const {data,team_data} = this.state
...
      })
      return(
        ...
        <Workbook filename="Teamsite.xlsx" element={
          ...
          <Workbook.Column label="Customer Title" value="customer"/>
          <Workbook.Column label="Origin Title" value="origin"/>
          <Workbook.Column label="Country Title" value="country"/>
  ...
  onChangeFileUpload(files)
    ...
    teamsiteService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
      ...
      const {locale,languageData,match} = this.props
        ...
        const columns = [
            ...
            onFilter: (value, record) => record.team === value,
             ...
             {
                title:  onChangeLanguage(locale,'Customer Title',languageData),
                dataIndex: 'customer',
                key: 'customer',
              ...
              {
                title:  onChangeLanguage(locale,'Origin Title',languageData),
                dataIndex: 'origin',
                key: 'origin',
              ...
              {
                title:  onChangeLanguage(locale,'Country Title',languageData),
                dataIndex: 'country',
                key: 'country',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Teamsite',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Teamsite',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Teamsite',languageData)}
                      </Button>
                    </div>
                  </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Teamsite' , languageData) : onChangeLanguage(locale,'Create Teamsite' , languageData) }   
          </ModalHeader>
          <ModalBody>
