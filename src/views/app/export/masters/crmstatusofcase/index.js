...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    crmstatusofcaseService.deletecrmstatusofcase(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Crmstatusofcase.xlsx" element={
          ...
          <Workbook.Column label="Listview" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    crmstatusofcaseService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'Status of Case',languageData),
                dataIndex: 'name',
                key: 'name',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Status Of Case',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Status Of Case',languageData)} match={match} />
                    </div>
                    ...
                    {/* <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Status Of Case',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Status Of Case' , languageData) : onChangeLanguage(locale,'Create Status Of Case' , languageData) }    
          </ModalHeader>
          <ModalBody>
