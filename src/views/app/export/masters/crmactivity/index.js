...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    crmactivityService.deletecrmactivity(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="CRM Activity.xlsx" element={
          ...
          <Workbook.Column label="CRM Activity" value="CRM Activity"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    crmactivityService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          if(res.data.status)   
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'CRM Activity',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-8">
                        <Breadcrumb heading= {onChangeLanguage(locale,'CRM Activity',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add CRM Activity',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update CRM Activity' , languageData) : onChangeLanguage(locale,'Create CRM Activity' , languageData) }   
           </ModalHeader>
          <ModalBody>
