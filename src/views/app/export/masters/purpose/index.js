...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    matchpurposeService.deletematchpurpose(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Purpose.xlsx" element={
          ...
          <Workbook.Column label="Purpose" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    matchpurposeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Purpose',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Purpose',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Purpose',languageData)}
                      </Button>
                    </div>
                  </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Purpose' , languageData) : onChangeLanguage(locale,'Create Purpose' , languageData) }     
