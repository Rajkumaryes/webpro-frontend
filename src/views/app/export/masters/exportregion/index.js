...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    regionexportService.deleteregion(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Region.xlsx" element={
          ...
          <Workbook.Column label="Region Code" value="region_code"/>
          <Workbook.Column label="Region Name" value="region_name"/>
  ...
  onChangeFileUpload(files)
    ...
    regionexportService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {  this.setState({
          loading : false
  ...
  handleReset = clearFilters => {
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
            {
              title:  onChangeLanguage(locale,'Region Code',languageData),
              dataIndex: 'region_code',
              key: 'region_code',
              ...this.getColumnSearchProps('region_code')
            ...
            },
             {
                ...
                key: 'region_name',
                ...this.getColumnSearchProps('region_name')
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'Region',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'Region',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add Region',languageData)}
                      </Button>
                    </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update Region' , languageData) : onChangeLanguage(locale,'Create Region' , languageData) }   
          </ModalHeader>
          <ModalBody>
