...
class MasterPage extends Component {
  ...
  handleDelete = id => {
    amttypeService.deleteamttype(id)
    .then((res) => {
      if(res){
...
      })
      return(
        ...
        <Workbook filename="Amttype.xlsx" element={
          ...
          <Workbook.Column label="Amttype" value="name"/>
          </Workbook.Sheet> 
        </Workbook>
  ...
  onChangeFileUpload(files)
    ...
    amttypeService.fileUpload(files[0])
      .then((res) => { 
        if(res.status)
        {
          this.setState({
  ...
  // };
    ...
    render()
    {
      const {locale,languageData,match} = this.props
        ...
        const columns = [
             {
                title:  onChangeLanguage(locale,'AMT Type',languageData),
                dataIndex: 'name',
                key: 'name',
        ...
        return (
            <>
            <title> {onChangeLanguage(locale,'AMT Type',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <div className = "row">
                    <div className = "col-md-7">
                        <Breadcrumb heading= {onChangeLanguage(locale,'AMT Type',languageData)} match={match} />
                    </div>
                    ...
                    <Button className = "button-width" color="primary">
                       ...
                       <Button color = "primary"  className = 'fontstyle button-width' onClick ={(event) => this.openModal()}>
                        {onChangeLanguage(locale,'Add AMT Type',languageData)}
                      </Button>
                    </div>
                  </div>
    ...
    return (
        ...
        >
          <ModalHeader toggle={()=>this.closeModal()}>
              {isEdit ? onChangeLanguage(locale,'Update AMT Type' , languageData) : onChangeLanguage(locale,'Create AMT Type' , languageData) }   
          </ModalHeader>
          <ModalBody>
