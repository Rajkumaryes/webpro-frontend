...
import { dpretypeService } from '../../../../redux/Export/masters/dpretype/saga';
...
class Sidebar extends Component {
    ...
    fetchIputingCount(){
        ...
        IndexingsheetService.fetchinputcount(username)
        .then((res) => {
           if(res.status)   { 
                  let filterstatus = res.data;
                  let lastdata = res.lastcount;
                  this.setState({ 
                    inputingcount:filterstatus, 
                    inputingcountlast:lastdata     
                  }) 
                 }
                 else{
  ...
  fetchdpre() {
    ...
    setValue(record) {
        ...
        if (record !== null && record) {
            this.setState({
                ...
                dc_responsible:record.dc_responsible,
    ...
    searchTeamSite() {
        ...
        console.log("1 country_code = ", country_code)
        console.log("2 issuer = ", issuer)
        console.log("3 shipper_coder = ", shipper_coder)
        console.log("4 mr_code = ", mr_code)
        console.log("5 consignee = ", consignee)
        ...
        if (country_code !== '' && issuer !== '') {
            ...
            console.log("9 teamdata = ", teamdata)
            this.setState({
                loading: true,
            })
    ...
    onChangeabot(value) {
        this.setState({ bot_activity: value })
    }
    handledpre = (selectedOptions) => {
        this.setState({dpre : selectedOptions.value})  
    }

    async onPasteD1040() {
        clipboard.readText().then((text) => {
            var record = getValue_D1040(text)
            // console.log(record, 'tetttttt')
            // console.log("kjbkj ", JSON.stringify(record))
             let final_dc_responsible = record.dc_responsible.trim();
            const validValues = ["DPRE_OK", "DPRE_MC", "DPRE_RU", "RCHECK", "RDONE", "CUS_CHK"];

            if (!validValues.includes(final_dc_responsible)) {
                final_dc_responsible = "DPRE_NOK";
            }
            this.setState({
                shipment_no: record.shipment_no,
                mtd_number: record.mtd_number,
                ...
                pod_end: record.pod_end,
                last_pod: record.last_pod,
                dc_responsible:final_dc_responsible
            })
            if (record.issuer !== "") {
    ...
    getteamvalue(value) {
        ...
        console.log("rajkumar ", this.state.team)
    }
    ...
    onSubmit() {
        const { start_time, no_of_container, no_of_cargoitem, no_of_hbl, mtd_number, customer,dpre,
            issuer, shipper_coder, mr_code, main_pod, doc_cutoff, aggregated_status, type, date, time, medium, shipment_no,
            bl, etd, main_pol, main_voyage, last_pod, auditsheet, aggregate_status, exception, shipment_type,dc_responsible,
            mtd_type, hbl, read, draft, sdc, free_time, credit, team, pod_end, consignee, frob_shipment, isadmin, updated_start_time,bot_activity } = this.state;


           let final_dc_responsible = dc_responsible.trim();
            const validValues = ["DPRE_OK", "DPRE_MC", "DPRE_RU", "RCHECK", "RDONE", "CUS_CHK"];
        ...
        if (is_fill === true) {
            ...
            IndexingsheetService.createIndexingsheet(username, time_taken, no_of_container, no_of_cargoitem, no_of_hbl, mtd_number, customer,
                ...
                mtd_type, hbl, read, draft, sdc, free_time, credit, team, pod_end, consignee, frob_shipment,bot_activity,final_dc_responsible,
    ...
    render() {
        const { user_id, start_time, end_time, time_taken, no_of_container, no_of_cargoitem, no_of_hbl, mtd_number, customer,inputingcount,inputingcountlast,
            issuer, shipper_coder, mr_code, main_pod, doc_cutoff, aggregated_status, type, date, time, medium, shipment_no,isDataPasted,dpre,dpredata,
            ...
            consignee, origin_requirements, country_requirements, customer_requirements,bot_activity,dc_responsible } = this.state
        ...
        return (
            <>
                <title>{onChangeLanguage(locale, 'Input Sheet', languageData)}</title>
                {loading &&
                    <div>
                    ...
                    <Colxx xxs="12">
                        ...
                        <div className="row">
                            <div className="col-md-8">
                                <Breadcrumb heading={onChangeLanguage(locale, 'Input Sheet', languageData)} match={match} />
                            </div>
                       ...
                       {isadmin === true ?this.renderTemplateadmin():this.renderTemplate()}
                        ...
                        <div className = "col-md-2" >
                        <h2 style = {{marginTop:'15px'}}>Total EQ : {inputingcount}</h2>
                        </div>
                        <div className = "col-md-2">
                            <h2  style = {{marginTop:'15px'}}>Last EQ : {inputingcount}</h2>
                        </div>
                        </div>
                            ...
                            <div className="row">
                                ...
                                <div className="col-lg-2-0 space-margin"  >
                                    ...
                                    <DatePickerDate
                                        ...
                                        selected={date}
                                        className="text-background-paste"
                                        onChange={(date) => this.setState({ date: date })} disabled={isDataPasted} 
                                    />
                                ...
                                <div className="col-lg-2-0 space-margin">
                                    ...
                                    <Input className={is_submit === true && time === '' ? "error-border-paste" : "fontstyle text-background-paste"}
                                        ...
                                        type='time'
                                        value={time} disabled={isDataPasted} 
                                ...
                                <div className="col-lg-2-0 space-margin">
                                    ...
                                    <Input className={is_submit === true && medium === '' ? "error-border-paste" : "fontstyle text-background-paste"}
                                        placeholder=''
                                        value={medium} disabled={isDataPasted} 
                                ...
                                <div className="col-lg-2-0 space-margin">
                                    ...
                                    <Input className={is_submit === true && bl === '' ? "error-border-paste" : "fontstyle text-background-paste"}
                                        placeholder=''
                                        value={bl} disabled={isDataPasted} 
                                ...
                                <div className="col-lg-2-0 space-margin">
                                    ...
                                    <DatePickerDate
                                        selected={etd}
                                        className="text-background-paste"
                                        onChange={(date) => this.setState({ etd: date })} disabled={isDataPasted} 
                                    />
                                </div>
                                <div className="col-lg-2-0 space-margin">
                                    ...
                                    <Input className={is_submit === true && main_pol === '' ? "error-border-paste" : "fontstyle text-background-paste"}
                                        placeholder=''
                                        value={main_pol} disabled={isDataPasted} 
                                ...
                                <div className="col-lg-2-0 space-margin">
                                    ...
                                    <Input className={is_submit === true && main_voyage === '' ? "error-border-paste" : "fontstyle text-background-paste"}
                                        placeholder=''
                                        value={main_voyage} disabled={isDataPasted} 
                            ...
                            />
                                ...
                                <div className="col-lg-2-0 space-margin">
                                    ...
                                    <Input className={is_submit === true && last_pod === '' ? "error-border-paste" : "fontstyle text-background-paste"}
                                        placeholder=''
                                        value={last_pod} disabled={isDataPasted}  
                                        onChange={(e) => this.setState({ last_pod: e.target.value })}
                                    />
                                </div>
                                <div className="col-lg-2-0 space-margin">
                                    <Label className="fontstyle normal-font" >{onChangeLanguage(locale, 'DPRE', languageData)} <a style={{ color: 'red' }}>*</a></Label>
                                    <Input className={is_submit === true && dc_responsible === '' ? "error-border-paste" : "fontstyle text-background-paste"}
                                        placeholder=''
                                        value={dc_responsible} disabled={isDataPasted}  
                                        onChange={(e) => this.setState({ dc_responsible: e.target.value })}
                                    />
                                </div>
                                ...
                                {/* <div className = "col-lg-2-0 space-margin">
                                    <Label  className = "fontstyle normal-font" >{onChangeLanguage(locale,'DPRE',languageData)}<a style = {{color :'red'}}>*</a></Label>
                                    <Select  
                                        className={is_submit === true && dpre === ''?  "error-border-select":"react-select fontstyle" }
                                        classNamePrefix="react-select"
                                        name="form-field-name"
                                        value={dpredata.filter(option =>option.value === dpre)}
                                        options={dpredata}
                                        onChange={this.handledpre}
                                    />
                                </div>  */}
                    ...
                    <div className="publishuser-card-component" style={{ padding: '10px', borderRadius: '10px', marginBottom: '15px' }}>
                        <div className="row">
                            ...
                            <div className="col-md-2">
                                ...
                                {/* <div className="col-md-2">
                                    <Label className="fontstyle normal-font" >{onChangeLanguage(locale, 'BOT', languageData)}
                                        <a style={{ color: 'red' }}>*</a></Label>
                                    <p className='fontstyle mandatory-label'>{is_submit === true && bot_activity === '' && `${onChangeLanguage(locale, 'Mandatory Field', languageData)}`}</p>
                                    <div style={{ margin: '10px 0px' }}>

                                        <Row>
                                            <Colxx xxs="4">
                                                <CustomRadioButton checked="Yes" name={onChangeLanguage(locale, 'Yes', languageData)} value={bot_activity}
                                                    onChangeRadio={this.onChangeabot.bind(this)} />

                                            </Colxx>
                                            <Colxx xxs="4">
                                                <CustomRadioButton checked="No" name={onChangeLanguage(locale, 'No', languageData)} value={bot_activity}
                                                    onChangeRadio={this.onChangeabot.bind(this)} />
