   ...
   fetchoblactivity() {
    ...
    .then((res) => {
       if(res.status)   { 
          let filterstatus = (res.data).filter(item => item.status === 1)
             var customlist = filterstatus.map(function(cusmaidid) {
                  return  {label : cusmaidid.name ,value : cusmaidid.id.toString(),text:cusmaidid.category};
               });  
              
                this.setState({
                oblactivitydata :  customlist,
                loading:false
                })
             }
             else{
          ...
          if ((Date.parse(enddate) > Date.parse(startdate)))
           {
            ...
            const report_url =  'levelsheet',
            menu= 'Export' , 
            submenu='Communication & Transmission Level Sheet',
            type = 'Raw Data'
  ...
  paginationOptions = {
    ...
    render()
    {
        ...
        const {enddate,startdate,team_id,team_data,is_submit,loading,data,page,pageSize,
          customtypedata,area_data,teamdata,oblactivitydata} = this.state
        var columns= [
          {
          ...
          {
            title: onChangeLanguage(locale,'Process Type',languageData),
          ...
          {
            title: onChangeLanguage(locale,'Customs Type',languageData),
            dataIndex: 'customs_type',
            key: 'customs_type',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {getValue(customtypedata,'value','label',text)}    
              </div>),           
          },
          {
          ...
          {
            title: onChangeLanguage(locale,'Transmission level',languageData),
            dataIndex: 'transmission_level',
            key: 'transmission_level',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {text}    
              </div>),           
          ...
          {
            title: onChangeLanguage(locale,'Arrival Time',languageData),
          ...
          {
            title: onChangeLanguage(locale,'Custom Declaration',languageData),
            dataIndex: 'custome_declaration',
            key: 'custome_declaration',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {text}    
              </div>),           
          ...
          },
          {
            ...
            dataIndex: 'category', // This will be derived dynamically
            key: 'category',
            render: (text, record, index) => {
              // Get category name using obl_activity or customs_activity
              const categoryValue = getValue(
                oblactivitydata, 
                'value', 
                'text', 
                record.obl_activity || record.customs_activity
              );
          ...
          {
            title: onChangeLanguage(locale,'OBL Activty',languageData),
            dataIndex: 'obl_activity',
            key: 'obl_activity',
            render: (text, record,index) => ( 
              <div  style = {{padding:'2px',width:'100px'}}>
               {getValue(oblactivitydata,'value','label',text)}    
              </div>),           
          },
          {
            title: onChangeLanguage(locale,'Customs Activty',languageData),
            dataIndex: 'customs_activity',
        ...
        return (
            <>
            <title>{onChangeLanguage(locale,'Communication & Transmission Level Sheet Raw Data',languageData)}</title>
            <Row>
              <Colxx xxs="12">
                <Breadcrumb heading={onChangeLanguage(locale,'Communication & Transmission Level Sheet Raw Data',languageData)} match={match} />
